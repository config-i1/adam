...
\caption{(\#fig:adamExampleNormal)An example of a well behaved data, point forecast and a 95\% prediction interval.}
\end{figure}
...
\begin{equation}
    ...
    (\#eq:RMSLE)
\end{equation}
It assumes that the actual values and the forecasts are positive and is \textbf{minimised by geometric mean}. I have added the exponentiation in the formula @ref(eq:RMSLE), which is sometimes omitted, bringing the metric to the original scale to have the same units as the actual values \(y_t\).
...
\caption{(\#fig:errorMeasuresrRMSEDist)Distribution of rRMSE on the original scale.}
\end{figure}
...
\caption{(\#fig:errorMeasuresrRMSEDistLog)Distribution of rRMSE on the log scale.}
\end{figure}
...
\caption{(\#fig:roExample01)Rolling origin performance of two forecasting methods}
\end{figure}
...
\caption{(\#fig:roExample02)Boxplots of error measures of two methods.}
\end{figure}
...
\caption{(\#fig:smallCompetitionBoxplot)Boxplot of RMSE for the artificial example}
\end{figure}
...
\caption{(\#fig:mcbForCompetition)MCB test results for small competition.}
\end{figure}
...
\caption{(\#fig:mcbForCompetitionLarge)MCB test results for large competition.}
\end{figure}
...
\caption{(\#fig:levelExample)Level of time series without any variability.}
\end{figure}
...
\caption{(\#fig:trendExample)Time series with a positive trend and no variability.}
\end{figure}
...
\caption{(\#fig:seasonalExample)Time series with a positive trend, seasonal pattern and no variability.}
\end{figure}
...
\caption{(\#fig:allExample)Time series with random errors.}
\end{figure}
...
\tightlist
\item
  Decide which of the models to use based on the type of seasonality in the data: additive @ref(eq:PureAdditive) or multiplicative @ref(eq:PureMultiplicative)
\item
...
\caption{(\#fig:naiveExample)A Random Walk example.}
\end{figure}
...
\caption{(\#fig:globalMeanExample)A global level example.}
\end{figure}
...
\caption{(\#fig:SMAExample)Examples of SMA time series and several SMA models of different orders applied to it.}
\end{figure}
...
\caption{(\#fig:RWDriftExample)Random Walk with drift data and the model applied to that data.}
\end{figure}
...
\caption{(\#fig:NaiveSeasonalExample)Seasonal Random Walk and Seasonal Naïve.}
\end{figure}
...
\caption{(\#fig:ETSTaxonomyAdditive)Time series corresponding to the additive error ETS models}
\end{figure}
...
\caption{(\#fig:ETSTaxonomyMultiplicative)Time series corresponding to the multiplicative error ETS models}
\end{figure}
...
\caption{(\#tab:ETSAdditiveError)Additive error ETS models}
\centering
...
\caption{(\#tab:ETSMultiplicativeError)Multiplicative error ETS models}
\centering
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{l|l|l|l}
\hline
...
\caption{(\#tab:ETSModelsForecasts)Point forecasts and expectations of ETS models. n.c.f. stands for "No Closed Form".}
\centering
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{l|l|l|l}
\hline
...
\caption{(\#fig:SESExample1)An example with a time series and SES forecast. \(\hat{\alpha}=0\)}
\end{figure}
...
\caption{(\#fig:SESExample2)An example with a time series and SES forecast. \(\hat{\alpha}=0.1\)}
\end{figure}
...
\caption{(\#fig:SESExamples)SES with different smoothing parameters applied to the same data.}
\end{figure}
...
\caption{(\#fig:SESExampleNaive)SES with \(\hat{\alpha}=1\).}
\end{figure}
...
\caption{(\#fig:SESExample3)SES with optimal smoothing parameter.}
\end{figure}
...
Now, \textbf{why is it called ``exponential''}? Because the same method can be represented in a different form, if we substitute \(\hat{y}_{t}\) in right hand side of @ref(eq:BrownMethod) by the formula for the previous step:
\begin{equation}
  \begin{aligned}
...
\caption{(\#fig:BrownExponentialExample)Example of weights distribution for \(\hat{\alpha}=0.25\)}
\end{figure}
...
\end{equation}
Cancelling out \(e_t\) and shifting everything by one step ahead, we obtain the error correction form @ref(eq:SESErrorCorrection) of SES. But now, the main benefit of having the model @ref(eq:ETSANN) instead of just the method @ref(eq:SESErrorCorrection) is in having a flexible framework, which allows adding other components, selecting the most appropriate ones, consistently estimating parameters \citep[see Section 4.3 of][]{SvetunkovSBA}, producing prediction intervals etc.
...
\caption{(\#fig:DGPetsANNExample)Local level data corresponding to ETS(A,N,N) model with different smoothing parameters.}
\end{figure}
...
\caption{(\#fig:ETSANNExample)An example of ETS(A,N,N) applied to the data generated from the same model.}
\end{figure}
...
\caption{(\#fig:ETSMNNExample)ETS(M,N,N) model applied to the data generated from the same model.}
\end{figure}
...
\caption{(\#fig:ETSAANExample)Data generated from ETS(A,A,N) model.}
\end{figure}
...
\caption{(\#fig:ETSAANExampleForecast)ETS(A,A,N) and a point forecast produced from it.}
\end{figure}
...
\caption{(\#fig:ETSAAdNExample)An example of ETS(A,Ad,N) data.}
\end{figure}
...
\caption{(\#fig:ETSAAdNExampleForecast)A point forecast from ETS(A,Ad,N).}
\end{figure}
...
\caption{(\#fig:ETSAAMExample)An example of ETS(A,A,M) data.}
\end{figure}
...
\caption{(\#fig:ETSAAMExampleForecast)A point forecast from ETS(A,A,M).}
\end{figure}
...
\caption{(\#fig:BJsalesDataPlot)Box-Jenkins sales data.}
\end{figure}
...
\caption{(\#fig:BJsalesAANForecast)Forecast for Box-Jenkins sales data from ETS(A,A,N) model.}
\end{figure}
...
\caption{(\#fig:BJsalesAANForecastAdmissible)Forecast for Box-Jenkins sales data from ETS(A,A,N) model with admissible bounds.}
\end{figure}
...
\caption{(\#fig:BJsalesAANForecastGN)Forecast for Box-Jenkins sales data from ETS(A,A,N) model with Generalised Normal distribution.}
\end{figure}
...
\caption{(\#fig:AirPassengersPlot)Air passengers data from Box-Jenkins textbook.}
\end{figure}
...
\caption{(\#fig:AirPassengersAAAForecast)Forecast for air passengers data using ETS(A,A,A) model.}
\end{figure}
...
\caption{(\#fig:AirPassengersAAADecomposition)Decomposition of air passengers data using ETS(A,A,A) model.}
\end{figure}
...
\caption{(\#fig:BJSalesadamETSMMN)Model fit for Box-Jenkins Sales data from ETS(M,M,N).}
\end{figure}
...
\caption{(\#fig:BJSalesadamETSMMNForecast)Forecast for Box-Jenkins Sales data from ETS(M,M,N).}
\end{figure}
...
\caption{(\#fig:AirPassengersMMMForecast)Forecast for air passengers data using ETS(M,M,M) model.}
\end{figure}
...
\caption{(\#fig:AirPassengersMMMDecomposition)Decomposition of air passengers data using ETS(M,M,M) model.}
\end{figure}
...
\caption{(\#fig:AirPassengersMMMACFPACF)ACF and PACF of residuals of ETS(M,M,M) model.}
\end{figure}
...
\begin{equation}
  ...
  (\#eq:ETSADAMMAM)
\end{equation}
The model @ref(eq:ETSADAMStateSpace) with different values for the functions is the basis of \texttt{adam()} function from \texttt{smooth} package.
...
\caption{(\#fig:plotsOfExponent)Plots of exponential increase and exponential decline.}
\end{figure}
...
\caption{(\#fig:AR1Trajectory)Forecast trajectory for AR(1) with \(\phi_1=0.9\).}
\end{figure}
...
\caption{(\#fig:AR3Trajectory)Forecast trajectory for an AR(3) model.}
\end{figure}
...
\caption{(\#fig:ARMAConstantTrajectory)Forecast trajectory for an ARMA(1,1) model with constant.}
\end{figure}
...
Another simple model that relies on differences of the data is called \textbf{Random Walk with drift} (which was discussed in Section @ref(RWWithDrift)) and is formulated by adding constant \(a_0\) to the right hand side of equation @ref(eq:ARIMARandomWalk):
\begin{equation}
...
\caption{(\#fig:SARIMATrajectory)Forecast trajectory for a SARIMA(1,0,1)(1,0,1)\(_4\).}
\end{figure}
...
\caption{(\#fig:SARIMAConstantTrajectory)Forecast trajectory for a SARIMA model with constant.}
\end{figure}
...
\caption{(\#fig:SARIMADiffConstantTrajectory)Forecast trajectory for a SARIMA model with differences and a constant.}
\end{figure}
...
\caption{(\#fig:ACFExampleAR1)ACF for AR(1) model.}
\end{figure}
...
\protect\hypertarget{exm:unnamed-chunk-38}{}{(\#exm:unnamed-chunk-38) }All birds have wings. Sarah has wings. Thus Sarah is a bird.

Here is Sarah:
\EndKnitrBlock{example}

\begin{figure}
\includegraphics[width=0.75\linewidth]{./images/09-Sarah} \caption{Sarah by Egor Kamelev}(\#fig:SarahButterfly)
\end{figure}
...
\caption{(\#fig:ETSARIMAConnection)ETS(A,N,N) and ARIMA(0,1,1) models producing the same fit and forecast trajectories.}
\end{figure}
...
\caption{(\#fig:BJSalesPlot)Box-Jenkins sales data.}
\end{figure}
...
\caption{(\#fig:adamARIMAPlotBJSales)BJSales series and ARIMA(0,2,2)}
\end{figure}
...
\caption{(\#fig:adamARIMAPlotBJSalesACFPACF)ACF and PACF of ARIMA(0,2,2) on BJSales data}
\end{figure}
...
\caption{(\#fig:adamSARIMAPlotAir)Forecast of AirPassengers data from SARIMA(0,2,2)(0,1,1)\(_{12}\) model.}
\end{figure}
...
\caption{(\#fig:adamETSPlotAirACFPACF)ACF and PACF of ETS(M,A,M) on AirPassengers data}
\end{figure}
...
\end{equation}
\citet{Hyndman2008b} show that the stability condition for SSOE models corresponds to the invertibility condition of ARIMA (Section @ref(MSARIMABounds)), so the model can either be checked via the discount matrix @ref(eq:ADAMARIMADiscountmatrix) or via the MA polynomials @ref(eq:MSARIMABoundsInvertibility).

When it comes to \textbf{stationarity}, state space ARIMA is always non-stationary if the differences \(\mathbf{D}_j \neq 0\) for any \(j\). So, there needs to be a different mechanism for the stationarity check. The simplest thing to do would be to expand the AR(p) polynomials, ignoring I(d), fill in the transition matrix \(\mathbf{F}\) and then calculate its eigenvalues. If they are lower than one by absolute value, the model is stationary. The same condition can be checked via the roots of the polynomial of AR(p) @ref(eq:MSARIMABoundsStationarity). However, the eigenvalues approach is more computationally efficient, and I recommend using it instead of the conventional polynomials calculation.
...
\end{equation}
which leads to an infinite combination of values of parameters \(\theta\) and \(\alpha\) that would produce exactly the same model fit. So, the model @ref(eq:ADAMETSARIMAANN011) does not have unique parameters and thus is \textbf{unidentifiable}. This means that we cannot reach the ``true model'' based on ETS(A,N,N)+ARIMA(0,1,1) and thus the model selection via information criteria becomes inappropriate. Furthermore, the estimates of parameters of such a model might become biased, inefficient and inconsistent due to the ``infinite combination'' issue mentioned above.
...
\caption{(\#fig:logSARIMAACF)ACF and PACF of logSARIMA(0,1,1)(0,1,1)\(_{12}\).}
\end{figure}
...
\caption{(\#fig:logSARIMAForecast)Forecast from logSARIMA(0,1,1)(0,1,1)\(_{12}\).}
\end{figure}
...
\caption{(\#fig:Seatbelts)The time series dynamics of variables from Seatbelts dataset.}
\end{figure}
...
\caption{(\#fig:SeatbeltsForecast)The actual values for drivers and a forecast from ETS(M,N,M) model.}
\end{figure}
...
\caption{(\#fig:driversSpread)The relation between variables from Seatbelts dataset}
\end{figure}
...
\caption{(\#fig:SeatbeltsForecastX)The actual values for drivers and a forecast from ETSX(M,N,M) model.}
\end{figure}
...
\caption{(\#tab:additiveErrorLikelihoods)Likelihood approach for additive error models}
\centering
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{l|l|l|l}
\hline
...
\caption{(\#tab:multiplicativeErrorLikelihoods)Likelihood approach for multiplicative error models}
\centering
\fontsize{12}{14}\selectfont
\begin{tabular}[t]{l|l|l|l}
\hline
...
\begin{equation}
  ...
  (\#eq:HAM)
\end{equation}
the minimum of which corresponds to the maximum of the likelihood of S distribution \citep[see Chapter 3 of][]{SvetunkovSBA}. The idea of this estimator is to minimise the errors that happen very often, close to the location of distribution. It will typically ignore the outliers and focus on the values that happen most often. As a result, if used for the integer values, the minimum of HAM would correspond to the mode of that distribution if it is unimodal. I do not have a proof of this property, but it becomes apparent, given that the square root in @ref(eq:HAM) would reduce the influence of all values lying above 1 and increase the values of everything that lies between (0, 1) (e.g.~\(\sqrt{0.16}=0.4\), but \(\sqrt{16}=4\)).
...
\caption{(\#fig:adamModelsForecasts)Forecasts from ETS(M,A,M) model estimated using different loss functions.}
\end{figure}
...
\begin{equation}
  ...
  (\#eq:logGPL)
\end{equation}
where \(\boldsymbol{\Sigma}\) is the conditional covariance matrix for the vector of variables \(\mathbf{y}_t=\begin{pmatrix} y_{t+1|t} & y_{t+2|t} & \ldots & y_{t+h|t} \end{pmatrix}\), \(\mathbf{Y}\) is the matrix consisting of \(\mathbf{y}_t\) for all \(t=1, \ldots, T-h\) and \(\mathbf{E_t}^{\prime} = \begin{pmatrix} \eta_{t+1|t} & \eta_{t+2|t} & \ldots & \eta_{t+h|t} \end{pmatrix}\) is the vector of 1 to \(h\) steps ahead forecast errors. \citet{Svetunkov2020Multistep} showed that the maximisation of the likelihood @ref(eq:logGPL) is equivalent to minimising the generalised variance of the error term, \(|\hat{\boldsymbol{\Sigma}}|\), where:
\begin{equation}
...
\caption{(\#fig:taylorSeries)Half-hourly electricity demand in England and Wales}
\end{figure}
...
\caption{(\#fig:adamModelETSMNM12)Half-hourly electricity demand data decomposition according to ETS(M,N,M){[}48,336{]}}
\end{figure}
...
\caption{(\#fig:adamModelETSMNM)The fit and the forecast of the ETS(M,N,M){[}48,336{]} model on half-hourly electricity demand data.}
\end{figure}
...
\caption{(\#fig:adamModelETSMNMGTMSE12)Half-hourly electricity demand data decomposition according to ETS(M,N,M){[}48,336{]} estimated with GTMSE.}
\end{figure}
...
\caption{(\#fig:adamModelARIMA)The fit and the forecast of the ARIMA(0,1,1)(0,1,1)\(_336\) model on half-hourly electricity demand data.}
\end{figure}
...
The general model @ref(eq:IntermittentDemandModel) assumes that demand occurs randomly and that the variable \(o_t\) can be either zero (no demand) or one (there is some demand). While this process can be modelled using different distributions, \citet{Svetunkov2019a} proposed using Bernoulli with a time varying probability (in the most general case):
\begin{equation}
...
\caption{(\#fig:intermittentExamplePlot)Example of intermittent demand data.}
\end{figure}
...
\caption{(\#fig:oETSFModel)Demand occurrence and probability of occurrence in the oETS\(_F\) model.}
\end{figure}
...
\caption{(\#fig:oETSOModel)Demand occurrence and probability of occurrence in the oETS(M,M,N)\(_O\) model.}
\end{figure}
...
\caption{(\#fig:oETSIModel)Demand occurrence and probability of occurrence in the oETS(M,M,N)\(_I\) model.}
\end{figure}
...
\caption{(\#fig:oETSGModel)Demand occurrence and probability of occurrence in the oETS(M,N,N)(A,A,N)\(_G\) model.}
\end{figure}
...
\caption{(\#fig:oETSDModel)Demand occurrence and probability of occurrence in the oETS(M,M,N)\(_D\) model.}
\end{figure}
...
\end{equation}
where \(\textbf{y}\) is the vector of all actual values and \(f_z(z_t | \mathbf{v}_{z,t-\mathbf{l}})\) can be substituted by a likelihood of the assumed distribution from the list of candidates in Section @ref(ADAMETSEstimationLikelihood). The main issue in calculating the likelihood @ref(eq:LogLikelihoodForDGP) is that the demand sizes are not observable when \(o_t=0\). This means that we cannot calculate the likelihood using the conventional approach, we need to use something else. \citet{Svetunkov2019a} proposed using Expectation Maximisation (EM) algorithm for this purpose, which is typically done in the following stages:
...
\caption{(\#tab:differentialEntropy)Differential entropies for different distributions. $\Gamma(\cdot)$ is the Gamma function, while $\psi(\cdot)$ is the digamma function.}
\centering
\fontsize{12}{14}\selectfont
...
\caption{(\#fig:adamModelsiETSBest)The fit of the best model to the intermittent data.}
\end{figure}
...
\caption{(\#fig:adamiETSForecasts)Point forecasts and prediction interval from the iETS(M,Md,N)\(_D\) model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat01)Basic regression model for the data of Road Casualties in Great Britain 1969--84.}
\end{figure}
...
\caption{(\#fig:carSeatsSpread)Spread plot for the residuals of model vs not included variables.}
\end{figure}
...
\caption{(\#fig:adamModelSeat02)The data and the fitted values for the second model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat02Resid)Standardised residuals vs time plot.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03)Diagnostic plots for Model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat04)Diagnostic plots for Model 4.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03ActualsFitted)Actuals vs Fitted for Model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03LinearPlot)Actuals and Fitted values for Model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03Resid)Standardised residuals vs Fitted for pure additive ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03ResidTime)Standardised residuals vs Time for pure additive ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05Resid)Standardised residuals vs Fitted for pure multiplicative ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05ResidFitted)Standardised residuals vs Fitted for the pure multiplicative ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05ResidTime)Standardised residuals vs Time for the pure multiplicative ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05Student)Studentised residuals analysis for the pure multiplicative ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05Outliers)Actuals over time with points corresponding to outliers of the pure multiplicative ETSX model.}
\end{figure}
...
\caption{(\#fig:adamModelSeat07Outliers)Standardised residuals vs Fitted for the model 7.}
\end{figure}
...
\caption{(\#fig:adamModelSeat09ResidvsTime)Standardised residuals vs time for the model 9.}
\end{figure}
...
\caption{(\#fig:adamModelSeat10ResidvsTime)Standardised residuals vs time for the model 10.}
\end{figure}
...
\caption{(\#fig:adamModelSeat10ACFPACF)ACF and PACF of residuals of the model 10.}
\end{figure}
...
\caption{(\#fig:adamModelSeat13ACFPACF)ACF and PACF of residuals of the model 13.}
\end{figure}
...
\caption{(\#fig:adamModelSeat16Resid)Residuals of the model 16.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03Hetero)Absolute and Squared residuals vs Fitted of model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03HeteroStd)Absolute and Squared standardised residuals vs Fitted of model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03Spread)Spread plot of absolute residuals vs variables included in the model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05HeteroPlot)Absolute and Squared residuals vs Fitted of model 5.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05Spread)Spread plot of absolute residuals vs variables included in the model 5.}
\end{figure}
...
\caption{(\#fig:adamModelSeat05Multistep)Boxplot of multistep forecast errors extracted from model 5.}
\end{figure}
...
\caption{(\#fig:adamModelSeat03QQ)QQ plot of residuals extracted from model 3.}
\end{figure}
...
\caption{(\#fig:adamModelSeat17QQ)QQ plot of residuals extracted from multiplicative model with normal distribution.}
\end{figure}
...
\caption{(\#fig:adamModelSeat18QQ)QQ plot of residuals extracted from multiplicative model with Inverse Gaussian distribution.}
\end{figure}
...
\caption{(\#fig:adamModelSeat3Density)Histogram and density line for the residuals from model 3 (assumed to follow Normal distribution).}
\end{figure}
...
\caption{(\#fig:adamARIMAModelFitExample)Actuals, fitted and forecast for the Box-Jenkins Sales data.}
\end{figure}
...
\caption{(\#fig:adamETSCCN)An example of combination of ETS non-seasonal models on Box-Jenkins sale time series.}
\end{figure}
...
\caption{(\#fig:adamETSCCNAAdN)An example of combination of ETS non-seasonal models on Box-Jenkins sale time series.}
\end{figure}
...
## 141 257.6672           254.9399            260.4123
## 142 257.7296           253.4438            262.0591
## 143 257.8036           251.9609            263.6744
## 144 257.8677           250.4531            265.2877
## 145 257.9195           248.8985            266.8763
## 146 257.9778           247.4409            268.5735
## 147 258.0305           245.8942            270.2246
## 148 258.0741           244.2543            271.9100
## 149 258.1235           242.6741            273.5596
## 150 258.1732           241.0731            275.4130
\end{verbatim}
...
\caption{(\#fig:adamCombinedfinalForecast)Final forecast from the combination of ETS, ARIMA and ETSX models.}
\end{figure}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.020  0.001  0.003  0.016 -0.021
## beta   0.001  0.005  0.000 -0.009  0.019
## phi    0.003  0.000  0.009  0.040 -0.044
## level  0.016 -0.009  0.040  0.470 -0.672
## trend -0.021  0.019 -0.044 -0.672  1.034
\end{verbatim}
...
Based on these values and the formula @ref(eq:confidenceInterval) we can produce confidence intervals for parameters of any ADAM model, which is done in R using \texttt{confint()} method. For xample, here are th intervals for the significance level of 1\%:
...
\caption{(\#fig:adamModelBJAAdN)Model fit and point forecasts of ETS(A,Ad,N) on Box-Jenkins Sales data.}
\end{figure}
...
\caption{(\#fig:adamETSRefitted)Refitted ADAM ETS(M,M,M) model on AirPassengers data.}
\end{figure}
...
\caption{(\#fig:adamARIMARefitted)Refitted ADAM ARIMA(0,1,1) model on artificial data.}
\end{figure}
...
\caption{(\#fig:adamARIMARefitted200)Refitted ADAM ARIMA(0,1,1) model on artificial data, bootstrapped covariance matrix.}
\end{figure}
...
\item
  Generate actual values from \(n\) models with the provided state vector, transition, measurement and persistence matrices and the generated error terms for the next \(h\) observations. In a general case, this is done using the model @ref(eq:ETSADAMStateSpace);
\item
...
\caption{(\#fig:adamForecastSimulated)Data generated from 1000 ETS(M,M,N) models.}
\end{figure}
...
If a model has multiplicative components or relies on a different distribution, then the several steps ahead actual value will not necessarily follow the assumed distribution and the formula @ref(eq:predictionIntervalGeneral) will produce incorrect intervals. For example, if we work with a pure multiplicative ETS model, ETS(M,N,N), assuming that \(\epsilon_t \sim \mathcal{N}(0, \sigma^2)\), the 2 steps ahead actual value will be expressed in terms of the values on observation \(t\):
\begin{equation}
...
\caption{(\#fig:adamModelDiagnostics)Diagnostics of the ADAM model on BJSales data.}
\end{figure}
...
\caption{(\#fig:adamModelPIComplete)Prediction intervals for ADAM on BJSales data.}
\end{figure}
...
\caption{(\#fig:adamModelPIRefitted)Refitted values for ADAM on BJSales data.}
\end{figure}
...
\caption{(\#fig:adamModelXPI6Plots)Different prediction intervals for ADAM ETS(M,N,N) on BJSales data}
\end{figure}
...
\caption{(\#fig:adamModelXForecastErrors)Distribution of in-sample multistep forecast errors from ADAM ETSX(M,N,N) model on BJSales data. Red point correspond to mean values.}
\end{figure}
...
In case of \textbf{pure additive ADAM models}, there are analytical formulae for the conditional expectations and conditional variance for this case that can be used in forecasting. These formulae come directly from the recursive relation @ref(eq:ETSADAMStateSpacePureAdditiveRecursion04) \citep[for derivations in a simpler case, see for example,][\citet{Svetunkov2017}]{Hyndman2008b}:
\begin{equation}
  \begin{aligned}
...
\caption{(\#fig:adamModeliETSCumulative)Cumulative demand, expectation and the upper bound based on iETS model.}
\end{figure}

What Figure @ref(fig:adamModeliETSCumulative) demonstrates is that for the holdout period of 7 days, the cumulative demand was around 27 units, while the upper bound of the interval was approximately 33. Based on that upper bound we could place an order (based on what we already have in stock) and have an appropriate safety stock.
...
\caption{(\#fig:adamModelConfidence)Confidence interval for the point forecast from ADAM ETS(A,Ad,N) model.}
\end{figure}
