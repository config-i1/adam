...
\usepackage{booktabs}
\usepackage{amsthm, graphicx, mathtools, amssymb, amsmath, amsbsy, unicode-math}
\makeatletter
\def\thm@space@setup{%
...
Figure \ref{fig:adamExampleNormal} shows an example, with a well behaved demand, for which the best point forecast is the straight line. In order to capture the uncertainty of demand, we can construct the prediction interval, which will tell in which bound the demand will lie in \(1-\alpha\) percent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 80.57 and 121.33. Capturing the uncertainty correctly is important, because the real life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.08001034 0.07051215 0.06180917
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.07754010 0.07057974 0.06187925
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 526.4958
## Error standard deviation: 32.1402
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1058.992 1059.222 1067.038 1067.578 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -2.2%; sCE: -22.8%; Asymmetry: -68.6%; MAPE: 3.1%
## MASE: 0.812; sMAE: 2.8%; sMSE: 0.1%; rMAE: 0.987; rRMSE: 0.988
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 696.5048
## Error standard deviation: 0.1038
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1399.010 1399.240 1407.056 1407.596 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 2.9%; sCE: 40.6%; Asymmetry: 41.5%; MAPE: 7.5%
## MASE: 0.833; sMAE: 7.4%; sMSE: 0.7%; rMAE: 0.821; rRMSE: 0.791
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.861584411  15.614094818 486.367497963  -0.016871546   0.034064647 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.267965878   0.059483983   0.007058802   0.648318992   0.703869862 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.205448616   0.214162782   0.082364184  -0.243390793   2.179105771
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
    & {y}_{t} = \mathbf{w}'_t \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \epsilon_t \\
    & \mathbf{v}_t = \mathbf{F} \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \mathbf{g} \epsilon_t
  \end{aligned}
...
\begin{equation}
  \begin{aligned}
    {y}_{t} = & \exp\left(\mathbf{w}'_t \log \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \log(1 + \epsilon_t)\right) \\
    \log \mathbf{v}_t = & \mathbf{F} \log \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \log(\mathbf{1}_k + \mathbf{g} \epsilon_t)
  \end{aligned}. 
...
\begin{equation}
  \begin{aligned}
    ...
    & \boldsymbol{\mathscr{l}} = \begin{pmatrix} 1 \\ 1 \\ m \\ 1 \\ \vdots \\ 1 \end{pmatrix}
  \end{aligned},
...
\begin{equation}
  \begin{aligned}
    & {y}_{t} = \mathbf{w}'_t \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \epsilon_t \\
    & \mathbf{v}_t = \mathbf{F} \mathbf{v}_{t-\boldsymbol{\mathscr{l}}} + \mathrm{diag}\left(\mathbf{w}_t\right)^{-1} \mathbf{g} \epsilon_t
  \end{aligned}
...
## 0.4406 0.9895 
## 
## Error standard deviation: 1.2742
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 120.0515 120.4324 130.8534 131.7487
\end{verbatim}
...
## 4.3266 0.9214 
## 
## Error standard deviation: 4.6364
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 122.3056 122.6865 133.1075 134.0028
\end{verbatim}
...
\begin{equation}
    \begin{aligned}
        ...
        & a_t = w_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) + r_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) \epsilon_{a,t} \\
        & \mathbf{v}_{a,t} = f_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) + g_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) \epsilon_{a,t} \\
        & b_t = w_b(\mathbf{v}_{b,t-\boldsymbol{\mathscr{l}}}) + r_b(\mathbf{v}_{b,t-\boldsymbol{\mathscr{l}}}) \epsilon_{b,t} \\
        & \mathbf{v}_{b,t} = f_b(\mathbf{v}_{b,t-\boldsymbol{\mathscr{l}}}) + g_b(\mathbf{v}_{b,t-\boldsymbol{\mathscr{l}}}) \epsilon_{b,t}
    \end{aligned} ,
  \label{eq:oETSG}
\end{equation}
...
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 123.7496 124.5651 139.9525 141.8692
\end{verbatim}
...
## -2.1289 
## 
## Error standard deviation: 1.9951
## Sample size: 110
## Number of estimated parameters: 2
## Number of degrees of freedom: 108
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 115.7496 115.8617 121.1506 121.4141
\end{verbatim}
...
## 0.3945 1.0041 
## 
## Error standard deviation: 0.9039
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 125.7472 126.1282 136.5491 137.4445
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
        ...
        & \hat{\mathbf{v}}_{t} = 
            \left \lbrace \begin{aligned}
                & f(\hat{\mathbf{v}}_{t-\boldsymbol{\mathscr{l}}}) + g(\hat{\mathbf{v}}_{t-\boldsymbol{\mathscr{l}}}) e_t, & \text{ when } o_t=1 \\
                & f(\hat{\mathbf{v}}_{t-\boldsymbol{\mathscr{l}}}) , & \text{ when } o_t=0
            \end{aligned} \right.
  \end{aligned},
...
\begin{equation}
    \begin{aligned}
        & y_t = o_t z_t , \\
        & {z}_{t} = w_z(\mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}) + r_z(\mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}) \epsilon_{z,t} \\
        & \mathbf{v}_{z,t} = f_z(\mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}) + g_z(\mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}) \epsilon_{z,t} \\
        & \\
        & o_t \sim \text{Bernoulli} \left(p_t \right) , \\
        & p_t = f_p(\mu_{a,t}, \mu_{b,t}) \\
        & a_t = w_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) + r_a(\mathbf{v}_{a,t-\boldsymbol{\mathscr{l}}}) \epsilon_{a,t} \\
...
\begin{equation}
    \mathcal{L}(\boldsymbol{\theta} | y_t, o_t=1) = p_t f_z(z_t | \mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}) ,
    \label{eq:LogLikelihoodOccurs}
\end{equation}
while in the opposite case it is:
\begin{equation}
    \mathcal{L}(\boldsymbol{\theta} | y_t, o_t=0) = (1-p_t) f_z(z_t | \mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}}),
...
\begin{equation}
    \begin{aligned}
    \ell(\boldsymbol{\theta} | \textbf{Y}) & = \sum_{o_t=1} \log f_z \left(z_{t} | \mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}} \right) + \sum_{o_t=0} \text{E} \left( \log f_z \left(z_{t} | \mathbf{v}_{z,t-\boldsymbol{\mathscr{l}}} \right) \right) \\
...
##           402.5336           402.1071           399.9852           412.0172
\end{verbatim}
...
## 0.1743 1.0624 
## 
## Error standard deviation: 0.9014
## Sample size: 110
## Number of estimated parameters: 5
## Number of degrees of freedom: 105
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 120.9767 121.5536 134.4791 135.8350
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 136.845
## ARMA parameters of the model:
## AR:
## phi1[1] 
## -0.3396 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 396.6667 397.4823 412.8696 414.7863 
## 
## Forecast errors:
## Asymmetry: 59.955%; sMSE: 111.225%; rRMSE: 0.99; sPIS: -3434.719%; sCE: 448.486%
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 133.0965
## Persistence vector g:
## alpha  beta 
##     0     0 
## Damping parameter: 1
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 389.1696 389.9852 405.3725 407.2892 
## 
## Forecast errors:
## Asymmetry: 34.48%; sMSE: 100.625%; rRMSE: 0.942; sPIS: -2091.402%; sCE: 155.122%
\end{verbatim}
...
## Occurrence model type: Direct
## Distribution assumed in the model: Mixture of Bernoulli and Gamma
## Loss function type: likelihood; Loss function value: 151.4139
## ARMA parameters of the model:
## AR:
## phi1[1] 
## -0.2825 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 421.8045 422.1855 432.6065 433.5018 
## 
## Forecast errors:
## Asymmetry: 59.008%; sMSE: 110.281%; rRMSE: 0.986; sPIS: -3377.448%; sCE: 438.117%
\end{verbatim}
...
## 141 257.6595           254.9247            260.4107
## 142 257.7128           253.3827            262.0606
## 143 257.7787           251.9863            263.6271
## 144 257.8319           250.5275            265.2853
## 145 257.8822           249.0486            266.8200
## 146 257.9273           247.4529            268.4496
## 147 257.9747           245.8494            270.0719
## 148 258.0410           244.3772            271.8774
## 149 258.0959           242.8166            273.5839
## 150 258.1501           241.0999            275.3609
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.010 -0.003  0.000  0.005 -0.011
## beta  -0.003  0.008 -0.002 -0.024  0.034
## phi    0.000 -0.002  0.013  0.055 -0.060
## level  0.005 -0.024  0.055  0.515 -0.692
## trend -0.011  0.034 -0.060 -0.692  1.000
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 23 units, while the upper bound of the interval was approximately 47. Based on that upper bound we could place an order (based on what we already have in stock) and have an appropriate safety stock.
