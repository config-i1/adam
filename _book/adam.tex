...
Figure \ref{fig:adamExampleNormal} shows an example, with a well behaved demand, for which the best point forecast is the straight line. In order to capture the uncertainty of demand, we can construct the prediction interval, which will tell in which bound the demand will lie in \(1-\alpha\) percent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 83.66 and 119.23. Capturing the uncertainty correctly is important, because the real life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.04350603 0.06267891 0.07318204
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.04171958 0.06168851 0.07363028
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 518.4322
## Error standard deviation: 29.8279
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1042.864 1043.095 1050.911 1051.451 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -2.9%; sCE: -28.9%; Asymmetry: -71.3%; MAPE: 3.8%
## MASE: 1.18; sMAE: 3.3%; sMSE: 0.2%; rMAE: 1.083; rRMSE: 1.122
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 677.7417
## Error standard deviation: 0.1001
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1361.483 1361.714 1369.530 1370.070 
## 
## 95% parametric prediction interval was constructed
## 92% of values are in the prediction interval
## Forecast errors:
## MPE: -9.1%; sCE: -100.7%; Asymmetry: -61.3%; MAPE: 14%
## MASE: 1.55; sMAE: 14.9%; sMSE: 2.6%; rMAE: 1.341; rRMSE: 1.104
\end{verbatim}
...
\end{equation}
I use spaces in equation \eqref{eq:ETSADAMAAA} to show how the matrix form is related to the general one. The positions of \(l_{t-1}\), \(b_{t-1}\) and \(s_{t-m}\) correspond to the ones in the matrices in \eqref{eq:ETSADAMAAAMatrixForm}. Now we can give names to each matrix and vector, for example:
\begin{equation}
  \begin{aligned}
    ...
    \mathbf{v}_{t} = \begin{pmatrix} l_t \\ b_t \\ s_t \end{pmatrix}, & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 1 \\ m \end{pmatrix} & \mathbf{v}_{t-\boldsymbol{\ell}} = \begin{pmatrix} l_{t-1} \\ b_{t-1} \\ s_{t-m} \end{pmatrix}
  \end{aligned}.
...
\begin{equation}
  \begin{aligned}
    {y}_{t} = &\mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}} + \epsilon_t \\
    \mathbf{v}_{t} = &\mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} \epsilon_t
  \end{aligned},
  \label{eq:ETSADAMStateSpacePureAdditive}
\end{equation}
where \(\mathbf{w}\) is the measurement vector, \(\mathbf{F}\) is the transition matrix, \(\mathbf{g}\) is the persistence vector and \(\mathbf{v}_{t-\boldsymbol{\ell}}\) is the vector of lagged components and \(\boldsymbol{\ell}\) is the vector of lags. The important thing to note is that ADAM model is based on the model discussed in Section \ref{ETSConventionalModelAdditive}, it is formulated using lags of components rather than the transition of them over time. This comes to the elements of the vector \(\boldsymbol{\ell}\). Just for the comparison, the conventional ETS(A,A,A), formulated according to \eqref{eq:ETSConventionalStateSpace} would have the following transition matrix (instead of \eqref{eq:ETSADAMAAAMatrices}):
\begin{equation}
...
\end{equation}
where \(\mathbf{v}_{t-h{l}}\) is the vector of previous states, given the lagged values \(\boldsymbol{\ell}\). In order to obtain the recursion, we need to split the measurement and persisitence vectors together with the transition matrix into parts for the same lags of components, leading to the following transition equation in \eqref{eq:ETSADAMStateSpacePureAdditiveRecursion01}:
\begin{equation}
  \begin{aligned}
...
In order to obtain both conditions, we need to use a reduced form of ETS by inserting the measurement equation in the transition equation via \(\epsilon_t= {y}_{t} - \mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}}\):
\begin{equation}
  \begin{aligned}
    \mathbf{v}_{t} = &\mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} \left({y}_{t} - \mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}} \right)\\
    = & \left(\mathbf{F} - \mathbf{g}\mathbf{w}^\prime \right) \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} {y}_{t} \\
  \end{aligned}.
...
In order to better understand what we plan to discuss later, we can take an example of ETS(A,N,N) model, for which \(\mathbf{F}=1\), \(\mathbf{w}=1\), \(\mathbf{g}=\alpha\), \(\mathbf{v}_t=l_t\) and \(\boldsymbol{\ell}=1\). Inserting these values into \eqref{eq:ETSADAMStateSpacePureAdditiveBackRecursion01}, we get:
\begin{equation}
  \begin{aligned}
...
\item
  ...
  Here \(\mu_{y,t} = \mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}}\), \(\sigma^2\) is the variance of the error term in logarithms and the \(-\frac{\sigma^2}{2}\) appears due to the restriction \(\text{E}(\epsilon_t)=0\).
\item
...
\begin{equation}
  \begin{aligned}
    {y}_{t} = &\mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}}\left(1 + \frac{\epsilon_t}{\mathbf{w}^\prime \mathbf{v}_{t-\boldsymbol{\ell}}}\right) \\
    \mathbf{v}_{t} = &\mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} \epsilon_t
  \end{aligned}.
...
\begin{equation}
  \begin{aligned}
        \log y_t = & \mathbf{w}' \log(\mathbf{v}_{t-\boldsymbol{\ell}}) + \log(1 + \epsilon_{t}) \\
        \log \mathbf{v}_{t} = & \mathbf{F} \log \mathbf{v}_{t-\boldsymbol{\ell}} + \log(\mathbf{1}_k + \mathbf{g} \epsilon_t)
  \end{aligned},
...
\begin{equation}
  \begin{aligned}
    \mathbf{w} = \begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix}, & \mathbf{F} = \begin{pmatrix} 1 & 1 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}, & \mathbf{g} = \begin{pmatrix} \alpha \\ \beta \\ \gamma \end{pmatrix}, \\
    \mathbf{v}_{t} = \begin{pmatrix} l_t \\ b_t \\ s_t \end{pmatrix}, & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 1 \\ m \end{pmatrix}, & \mathbf{1}_k = \begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix}
  \end{aligned}.
...
##  -6.46902563  15.51109826 489.38803606  -0.01813057   0.03396521  -0.29573542 
##         sMAE         sMSE         MASE        RMSSE         rMAE        rRMSE 
##   0.05909160   0.00710264   0.64404243   0.70605214   0.20409340   0.21482677 
##         rAME    asymmetry         sPIS 
##   0.09089966  -0.28490022   2.34601867
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
    {y}_{t} = &w(\mathbf{v}_{t-\boldsymbol{\ell}}) + r(\mathbf{v}_{t-\boldsymbol{\ell}}) \epsilon_t \\
    \mathbf{v}_{t} = &f(\mathbf{v}_{t-\boldsymbol{\ell}}) + g(\mathbf{v}_{t-\boldsymbol{\ell}}) \epsilon_t
  \end{aligned},
  \label{eq:ETSADAMStateSpace}
\end{equation}
where \(\mathbf{v}_{t-\boldsymbol{\ell}}\) is the vector of lagged components and \(\boldsymbol{\ell}\) is the vector of lags, while all the other functions corresponds to the ones used in \eqref{eq:ETSConventionalStateSpace}. This model form is mainly useful for the formulation, rather than for the derivations, as discussed in Section \ref{ETSConventionalModel}. Not only it encompasses any pure models, it also allows formulating any of the mixed ones. For example, the ETS(M,A,M) will have the following values:
\begin{equation}
  \begin{aligned}
    w(\mathbf{v}_{t-\boldsymbol{\ell}}) = (l_{t-1}+b_{t-1}) s_{t-m}\text{, } &
    r(\mathbf{v}_{t-\boldsymbol{\ell}}) = w(\mathbf{v}_{t-\boldsymbol{\ell}}), \\
    f(\mathbf{v}_{t-\boldsymbol{\ell}}) = \begin{pmatrix} l_{t-1} + b_{t-1} \\ b_{t-1} \\ s_{t-m} \end{pmatrix}\text{, } &
    g(\mathbf{v}_{t-\boldsymbol{\ell}}) = \begin{pmatrix} \alpha (l_{t-1} + b_{t-1}) \\ \beta (l_{t-1} + b_{t-1}) \\ \gamma s_{t-m} \end{pmatrix}, \\
    \mathbf{v}_{t} = \begin{pmatrix} l_t \\ b_t \\ s_t \end{pmatrix}\text{, } &
    \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 1 \\ m \end{pmatrix}, \\
    \mathbf{v}_{t-\boldsymbol{\ell}} = \begin{pmatrix} l_{t-1} \\ b_{t-1} \\ s_{t-m} \end{pmatrix}
  \end{aligned}.
...
\begin{equation*}
  \begin{aligned}
    &{y}_{t} = \mathbf{w}' \mathbf{v}_{t-\boldsymbol{\ell}} + \epsilon_t \\
    &\mathbf{v}_{t} = \mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} \epsilon_t
  \end{aligned},
\end{equation*}
...
\begin{equation}
  \begin{aligned}
    ...
    \mathbf{g} = \begin{pmatrix} \eta_1 + \theta_1 \\ \eta_2 + \theta_2 \\ \vdots \\ \eta_K + \theta_K \end{pmatrix}, & \mathbf{v}_{t} = \begin{pmatrix} v_{1,t} \\ v_{2,t} \\ \vdots \\ v_{K,t} \end{pmatrix}, & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 2 \\ \vdots \\ K \end{pmatrix}
  \end{aligned}.
...
\begin{equation}
  \begin{aligned}
    ...
    \mathbf{g} = \begin{pmatrix} \eta_1 + \theta_1 \\ \eta_2 + \theta_2 \\ \vdots \\ \eta_K + \theta_K \\ 0 \end{pmatrix}, & \mathbf{v}_{t} = \begin{pmatrix} v_{1,t} \\ v_{2,t} \\ \vdots \\ v_{K,t} \\ v_{K+1,t} \end{pmatrix}, & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 2 \\ \vdots \\ K \\ 1 \end{pmatrix}
  \end{aligned}.
...
\begin{equation}
  \begin{aligned}
    ...
    \mathbf{g} = \begin{pmatrix} \alpha \\ \gamma \\ \phi_1 \\ \phi_2 \end{pmatrix}, & \mathbf{v}_{t} = \begin{pmatrix} l_t \\ s_t \\ v_{1,t-1} \\ v_{2,t-2} \end{pmatrix}, & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ m \\ 1 \\ 2\end{pmatrix}
  \end{aligned}.
...
\begin{equation}
  \begin{aligned}
    & {y}_{t} = \mathbf{w}'_t \mathbf{v}_{t-\boldsymbol{\ell}} + \epsilon_t \\
    & \mathbf{v}_t = \mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathbf{g} \epsilon_t
  \end{aligned}
...
\begin{equation}
  \begin{aligned}
    {y}_{t} = & \exp\left(\mathbf{w}'_t \log \mathbf{v}_{t-\boldsymbol{\ell}} + \log(1 + \epsilon_t)\right) \\
    \log \mathbf{v}_t = & \mathbf{F} \log \mathbf{v}_{t-\boldsymbol{\ell}} + \log(\mathbf{1}_k + \mathbf{g} \epsilon_t)
  \end{aligned}. 
...
\begin{equation}
  \begin{aligned}
    ...
    & \boldsymbol{\ell} = \begin{pmatrix} 1 \\ 1 \\ m \\ 1 \\ \vdots \\ 1 \end{pmatrix}
  \end{aligned},
...
\begin{equation}
  \begin{aligned}
    & {y}_{t} = \mathbf{w}'_t \mathbf{v}_{t-\boldsymbol{\ell}} + \epsilon_t \\
    & \mathbf{v}_t = \mathbf{F} \mathbf{v}_{t-\boldsymbol{\ell}} + \mathrm{diag}\left(\mathbf{w}_t\right)^{-1} \mathbf{g} \epsilon_t
  \end{aligned}
...
\begin{equation}
  \boldsymbol{\ell}'=\begin{pmatrix}1 & 1 & m_1 & m_2 & \dots & m_n \end{pmatrix},
...
## 0.7091 
## 
## Error standard deviation: 1.0801
## Sample size: 110
## Number of estimated parameters: 1
## Number of degrees of freedom: 109
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 134.6520 134.6890 137.3525 137.4395
\end{verbatim}
...
The plot above demonstrates the dynamics of the occurrence variable \(o_t\) and the fitted and predicted probabilities. The oETS\(_F\) model produces the straight line for the probability of 0.71, ignoring the fact that in our example the probability of occurrence has increased over time.
...
## 0.2067 1.0533 
## 
## Error standard deviation: 1.4505
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 100.0989 100.4799 110.9008 111.7962
\end{verbatim}
...
## 11.5756  0.9080 
## 
## Error standard deviation: 5.3346
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 101.4704 101.8513 112.2723 113.1676
\end{verbatim}
...
\begin{equation}
    \begin{aligned}
        ...
        & a_t = w_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) + r_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) \epsilon_{a,t} \\
        & \mathbf{v}_{a,t} = f_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) + g_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) \epsilon_{a,t} \\
        & b_t = w_b(\mathbf{v}_{b,t-\boldsymbol{\ell}}) + r_b(\mathbf{v}_{b,t-\boldsymbol{\ell}}) \epsilon_{b,t} \\
        & \mathbf{v}_{b,t} = f_b(\mathbf{v}_{b,t-\boldsymbol{\ell}}) + g_b(\mathbf{v}_{b,t-\boldsymbol{\ell}}) \epsilon_{b,t}
    \end{aligned} ,
  \label{eq:oETSG}
\end{equation}
...
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 105.1008 105.9163 121.3036 123.2203
\end{verbatim}
...
## 0.9953 
## 
## Error standard deviation: 1.6733
## Sample size: 110
## Number of estimated parameters: 2
## Number of degrees of freedom: 108
## Information criteria: 
##      AIC     AICc      BIC     BICc 
##  97.1008  97.2129 102.5017 102.7653
\end{verbatim}
...
## 0.2970 1.0048 
## 
## Error standard deviation: 1.1047
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 107.6129 107.9938 118.4148 119.3101
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
        ...
        & \hat{\mathbf{v}}_{t} = 
            \left \lbrace \begin{aligned}
                & f(\hat{\mathbf{v}}_{t-\boldsymbol{\ell}}) + g(\hat{\mathbf{v}}_{t-\boldsymbol{\ell}}) e_t, & \text{ when } o_t=1 \\
                & f(\hat{\mathbf{v}}_{t-\boldsymbol{\ell}}) , & \text{ when } o_t=0
            \end{aligned} \right.
  \end{aligned},
...
\begin{equation}
    \begin{aligned}
        & y_t = o_t z_t , \\
        & {z}_{t} = w_z(\mathbf{v}_{z,t-\boldsymbol{\ell}}) + r_z(\mathbf{v}_{z,t-\boldsymbol{\ell}}) \epsilon_{z,t} \\
        & \mathbf{v}_{z,t} = f_z(\mathbf{v}_{z,t-\boldsymbol{\ell}}) + g_z(\mathbf{v}_{z,t-\boldsymbol{\ell}}) \epsilon_{z,t} \\
        & \\
        & o_t \sim \text{Bernoulli} \left(p_t \right) , \\
        & p_t = f_p(\mu_{a,t}, \mu_{b,t}) \\
        & a_t = w_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) + r_a(\mathbf{v}_{a,t-\boldsymbol{\ell}}) \epsilon_{a,t} \\
...
\begin{equation}
    \mathcal{L}(\boldsymbol{\theta} | y_t, o_t=1) = p_t f_z(z_t | \mathbf{v}_{z,t-\boldsymbol{\ell}}) ,
    \label{eq:LogLikelihoodOccurs}
\end{equation}
while in the opposite case it is:
\begin{equation}
    \mathcal{L}(\boldsymbol{\theta} | y_t, o_t=0) = (1-p_t) f_z(z_t | \mathbf{v}_{z,t-\boldsymbol{\ell}}),
...
\begin{equation}
    \begin{aligned}
    \ell(\boldsymbol{\theta} | \textbf{Y}) & = \sum_{o_t=1} \log f_z \left(z_{t} | \mathbf{v}_{z,t-\boldsymbol{\ell}} \right) + \sum_{o_t=0} \text{E} \left( \log f_z \left(z_{t} | \mathbf{v}_{z,t-\boldsymbol{\ell}} \right) \right) \\
...
##           399.1889           395.5082           398.6307           409.4564
\end{verbatim}
...
## 7.9549 0.9176 
## 
## Error standard deviation: 4.5871
## Sample size: 110
## Number of estimated parameters: 5
## Number of degrees of freedom: 105
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 101.9153 102.4923 115.4177 116.7736
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 147.5454
## ARMA parameters of the model:
## AR:
## phi1[1] 
## -0.3879 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 399.0061 399.8216 415.2090 417.1257 
## 
## Forecast errors:
## Asymmetry: -9.569%; sMSE: 154.517%; rRMSE: 0.999; sPIS: -1108.474%; sCE: 41.414%
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 140.3887
## Persistence vector g:
##  alpha   beta 
## 0.0196 0.0105 
## Damping parameter: 0.9566
## Sample size: 110
## Number of estimated parameters: 6
## Number of degrees of freedom: 104
## Number of provided parameters: 5
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 384.6927 385.5082 400.8956 402.8123 
## 
## Forecast errors:
## Asymmetry: -25.757%; sMSE: 163.462%; rRMSE: 1.027; sPIS: 281.813%; sCE: -238.746%
\end{verbatim}
...
## Occurrence model type: Inverse odds ratio
## Distribution assumed in the model: Mixture of Bernoulli and Gamma
## Loss function type: likelihood; Loss function value: 160.3734
## ARMA parameters of the model:
## AR:
## phi1[1] 
##  0.1842 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 420.6622 421.0432 431.4641 432.3594 
## 
## Forecast errors:
## Asymmetry: -9.307%; sMSE: 154.49%; rRMSE: 0.999; sPIS: -1134.218%; sCE: 45.609%
\end{verbatim}
...
## 141 257.6597           254.9111            260.3942
## 142 257.7073           253.3901            262.0334
## 143 257.7535           251.9040            263.6396
## 144 257.7966           250.4169            265.2630
## 145 257.8380           248.9503            266.8172
## 146 257.8976           247.4193            268.5837
## 147 257.9377           245.7527            270.1964
## 148 257.9838           244.2459            271.9065
## 149 258.0336           242.6618            273.7139
## 150 258.0818           240.8400            275.4111
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.015  0.003  0.002  0.009 -0.009
## beta   0.003  0.010 -0.002 -0.017  0.028
## phi    0.002 -0.002  0.012  0.048 -0.050
## level  0.009 -0.017  0.048  0.522 -0.725
## trend -0.009  0.028 -0.050 -0.725  1.096
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 24 units, while the upper bound of the interval was approximately 39. Based on that upper bound we could place an order (based on what we already have in stock) and have an appropriate safety stock.
