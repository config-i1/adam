...
Figure \ref{fig:adamExampleNormal} shows an example, with a well behaved demand, for which the best point forecast is the straight line. In order to capture the uncertainty of demand, we can construct the prediction interval, which will tell in which bound the demand will lie in \(1-\alpha\) percent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 78.58 and 120.28. Capturing the uncertainty correctly is important, because the real life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10139855 0.09808986 0.09584215
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 528.3615
## Error standard deviation: 32.7003
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1062.723 1062.954 1070.770 1071.310 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -2.1%; sCE: -22.5%; Asymmetry: -45.8%; MAPE: 4.3%
## MASE: 1.255; sMAE: 4.3%; sMSE: 0.2%; rMAE: 0.878; rRMSE: 0.83
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 667.2254
## Error standard deviation: 0.1142
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1340.451 1340.682 1348.497 1349.037 
## 
## 95% parametric prediction interval was constructed
## 92% of values are in the prediction interval
## Forecast errors:
## MPE: 12.6%; sCE: 222.4%; Asymmetry: 100%; MAPE: 12.6%
## MASE: 1.589; sMAE: 18.5%; sMSE: 5.5%; rMAE: 0.824; rRMSE: 0.879
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
    ...
    \mathbf{v}_{t} = \begin{pmatrix} l_t \\ b_t \\ s_t \end{pmatrix}, & \mathbf{l} = \begin{pmatrix} 1 \\ 1 \\ m \end{pmatrix} & \mathbf{v}_{t-\mathbf{l}} = \begin{pmatrix} l_{t-1} \\ b_{t-1} \\ s_{t-m} \end{pmatrix}
  \end{aligned}.
...
\end{equation}
where \(\mathbf{v}_{t-h{l}}\) is the vector of previous states, given the lagged values \(\mathbf{l}\). In order to obtain the recursion, we need to split the measurement and persisitence vectors together with the transition matrix into parts for the same lags of components, leading to the following transition equation in \eqref{eq:ETSADAMStateSpacePureAdditiveRecursion01}:
\begin{equation}
  \begin{aligned}
...
In order to better understand what we plan to discuss later, we can take an example of ETS(A,N,N) model, for which \(\mathbf{F}=1\), \(\mathbf{w}=1\), \(\mathbf{g}=\alpha\), \(\mathbf{v}_t=l_t\) and \(\mathbf{l}=1\). Inserting these values into \eqref{eq:ETSADAMStateSpacePureAdditiveBackRecursion01}, we get:
\begin{equation}
  \begin{aligned}
...
\item
  ...
  Here \(\mu_{y,t} = \mathbf{w}^\prime \mathbf{v}_{t-\mathbf{l}}\), \(\sigma^2\) is the variance of the error term in logarithms and the \(-\frac{\sigma^2}{2}\) appears due to the restriction \(\text{E}(\epsilon_t)=0\).
\item
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.668428947  15.565179275 484.141514301  -0.016465484   0.033920004 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.259135659   0.059297632   0.007026496   0.646287951   0.702257296 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204804990   0.213672135   0.079650055  -0.231200192   2.159655001
\end{verbatim}
...
## Information criteria: 
##      AIC     AICc      BIC     BICc 
##  92.3402  92.7212 103.1421 104.0375
\end{verbatim}
...
## 0.6816 0.8417 
## 
## Error standard deviation: 2.6641
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
## 292.0449 292.4259 302.8468 303.7421
\end{verbatim}
...
## Information criteria: 
##      AIC     AICc      BIC     BICc 
##  98.3286  99.1442 114.5315 116.4482
\end{verbatim}
...
## 0.5386 
## 
## Error standard deviation: 2.0109
## Sample size: 110
## Number of estimated parameters: 2
## Number of degrees of freedom: 108
## Information criteria: 
##     AIC    AICc     BIC    BICc 
## 90.3286 90.4408 95.7296 95.9932
\end{verbatim}
...
## 0.2868 1.0153 
## 
## Error standard deviation: 0.8015
## Sample size: 110
## Number of estimated parameters: 4
## Number of degrees of freedom: 106
## Information criteria: 
##      AIC     AICc      BIC     BICc 
##  96.0273  96.4083 106.8293 107.7246
\end{verbatim}
...
##           381.4089           390.2903           381.7703           399.1914
\end{verbatim}
...
## 0.3269 0.9750 
## 
## Error standard deviation: 1.6985
## Sample size: 110
## Number of estimated parameters: 5
## Number of degrees of freedom: 105
## Information criteria: 
##      AIC     AICc      BIC     BICc 
##  94.5240  95.1010 108.0264 109.3823
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 134.2986
## ARMA parameters of the model:
## AR:
## phi1[1] 
## -0.2554 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 365.1212 365.9367 381.3241 383.2408 
## 
## Forecast errors:
## Asymmetry: 55.713%; sMSE: 129.62%; rRMSE: 0.939; sPIS: -3258.732%; sCE: 591.976%
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 137.0347
## Persistence vector g:
##  alpha   beta 
## 0.0575 0.0000 
## Damping parameter: 1
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 370.5933 371.4089 386.7962 388.7129 
## 
## Forecast errors:
## Asymmetry: 30.332%; sMSE: 102.621%; rRMSE: 0.835; sPIS: -1718.359%; sCE: 274.759%
\end{verbatim}
...
## Occurrence model type: Odds ratio
## Distribution assumed in the model: Mixture of Bernoulli and Gamma
## Loss function type: likelihood; Loss function value: 147.9119
## ARMA parameters of the model:
## AR:
## phi1[1] 
## -0.1053 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 388.3478 388.7287 399.1497 400.0450 
## 
## Forecast errors:
## Asymmetry: 57.074%; sMSE: 130.754%; rRMSE: 0.943; sPIS: -3319.359%; sCE: 604.913%
\end{verbatim}
...
## 141 257.6543           254.9666            260.3779
## 142 257.7146           253.4110            262.0299
## 143 257.7675           251.9205            263.5859
## 144 257.8201           250.4712            265.1344
## 145 257.8649           249.0347            266.7490
## 146 257.9077           247.4251            268.4202
## 147 257.9589           245.9390            270.1426
## 148 258.0072           244.3545            271.7619
## 149 258.0479           242.6716            273.5509
## 150 258.0881           241.1170            275.3313
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.006  0.003  0.001  0.005 -0.005
## beta   0.003  0.006 -0.001 -0.012  0.018
## phi    0.001 -0.001  0.014  0.061 -0.065
## level  0.005 -0.012  0.061  0.533 -0.714
## trend -0.005  0.018 -0.065 -0.714  1.033
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 25 units, while the upper bound of the interval was approximately 37. Based on that upper bound we could place an order (based on what we already have in stock) and have an appropriate safety stock.
