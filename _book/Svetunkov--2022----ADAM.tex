...
Different error measures can be used in this case. Which one to use depends on the specific need. Here we briefly discuss the most important measures and refer reader to \citep{Davydenko2013, SvetunkovAccuracy2019, SvetunkovAPEs2017} for the gory details.
...
Finally, we write a simple loop that repeats the model fit and forecasting for the horizon \texttt{h} several times. The trickiest part is understanding how to define the train and test samples. In our example, the former should have \texttt{obs-origins+1-h} observations in the first step and \texttt{obs-h} in the last one so that we can have \texttt{h} observations in the test set throughout all origins, and we can repeat this \texttt{origins} times. One way of doing this is via the following loop:
...
\NormalTok{    testModel }\OtherTok{\textless{}{-}} \FunctionTok{sma}\NormalTok{(y[}\DecValTok{1}\SpecialCharTok{:}\NormalTok{(obs}\SpecialCharTok{{-}}\NormalTok{origins}\SpecialCharTok{+}\NormalTok{i}\SpecialCharTok{{-}}\NormalTok{h)])}
...
In the ideal situation, the boxplots in Figure \ref{fig:roOrigins} should be similar, meaning that the model performs consistently over different origins. However, more useful information would be obtained if the performance of one model is compared to the others in the rolling origin experiment.

\hypertarget{rolling-origin-function-in-r}{%
\subsection{Rolling origin function in R}\label{rolling-origin-function-in-r}}

In R, there are several packages and functions that implement rolling origin. One of those is the function \texttt{ro()} from the \texttt{greybox} package (written by Yves Sagaert and Ivan Svetunkov in 2016 on the way to the International Symposium on Forecasting). It implements the rolling origin evaluation for any function you like with a predefined \texttt{call} and returns the desired \texttt{value}. It heavily relies on the two variables: \texttt{call} and \texttt{value} -- so it is pretty important to understand how to formulate them to get the desired results. \texttt{ro()} is a very flexible function, but as a result, it is not very simple. In this subsection, we will see how it works on a couple of examples.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.08187663 0.08868345 0.07371749
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.09157130 0.09270062 0.06300703
\end{verbatim}
...
The boxplots in Figure \ref{fig:roExample02} can be interpreted as any other boxplots applied to random variables \citep[see, for example, discussion in Section 5.2 of][]{SvetunkovSBA}.

\begin{remark}

When it comes to applying \texttt{ro()} for regression-like models, one can use the internal parameters \texttt{counti}, \texttt{counto} and \texttt{countf}, which define the size of the in-sample, the holdout and the full sample, respectively. An example of the code in this situation is shown below with a function \texttt{alm()} from \texttt{greybox} package being used for fitting a simple linear regression model.
...
\CommentTok{\# Predict values from the model. counti and counto determine sizes for the in{-}sample and the holdout}
...
## Loss function type: likelihood; Loss function value: 515.1306
...
##      AIC     AICc      BIC     BICc 
## 1036.261 1036.492 1044.308 1044.848 
## 
## Forecast errors:
## ME: -1.44; MAE: 14.74; RMSE: 18.249
## sCE: -1.931%; Asymmetry: -13.9%; sMAE: 1.648%; sMSE: 0.042%
## MASE: 0.578; RMSSE: 0.559; rMAE: 0.341; rRMSE: 0.389
\end{verbatim}
...
## Distribution assumed in the model: Normal
## Loss function type: likelihood; Loss function value: 633.1149
## Persistence vector g:
##  alpha 
## 0.1734 
## 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1272.230 1272.460 1280.276 1280.816 
## 
## Forecast errors:
## ME: -16.044; MAE: 52.756; RMSE: 60.601
## sCE: -25.183%; Asymmetry: -37.7%; sMAE: 6.901%; sMSE: 0.628%
## MASE: 0.572; RMSSE: 0.547; rMAE: 0.355; rRMSE: 0.379
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.897034861  15.408553878 483.046097682  -0.016897363   0.033629918 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.269586517   0.058700947   0.007010597   0.639784646   0.701462384 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.202744130   0.213430271   0.082862317  -0.253505586   2.239814773
\end{verbatim}
...
## 141 257.6696           254.8985            260.4493
## 142 257.7258           253.3243            262.0974
## 143 257.7755           251.9063            263.6936
## 144 257.8248           250.3839            265.2572
## 145 257.8811           248.8686            266.8680
## 146 257.9186           247.3349            268.5857
## 147 257.9690           245.7570            270.2594
## 148 258.0121           244.1580            271.9932
## 149 258.0579           242.6049            273.6560
## 150 258.1051           241.0568            275.4252
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.015  0.004  0.002  0.007 -0.007
## beta   0.004  0.007  0.000 -0.003  0.009
## phi    0.002  0.000  0.014  0.065 -0.071
## level  0.007 -0.003  0.065  0.555 -0.717
## trend -0.007  0.009 -0.071 -0.717  1.001
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 15 units, while the upper bound of the interval was approximately 52. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
