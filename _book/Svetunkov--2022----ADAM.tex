...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 78.73 and 115.25. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.07417614 0.07290728 0.06884374
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.07496907 0.07680397 0.07058098
\end{verbatim}
...
\hline
Trend="N" & $\begin{aligned} &y_{t} = l_{t-1} + \epsilon_t \\
    ...
    \end{aligned}$\\
\hline
Trend="A" & $\begin{aligned} &y_{t} = l_{t-1} + b_{t-1} + \epsilon_t \\
    ...
    \end{aligned}$\\
\hline
Trend="Ad" & $\begin{aligned} &y_{t} = l_{t-1} + \phi b_{t-1} + \epsilon_t \\
    ...
    \end{aligned}$\\
\hline
Trend="M" & $\begin{aligned} &y_{t} = l_{t-1} b_{t-1} + \epsilon_t \\
      &l_t = l_{t-1} b_{t-1} + \alpha \epsilon_t \\
    ...
    \end{aligned}$\\
\hline
Trend="Md" & $\begin{aligned} &y_{t} = l_{t-1} b_{t-1}^\phi + \epsilon_t \\
...
\hline
  & Seasonal="N" & Seasonal="A" & Seasonal="M"\\
\hline
Trend="N" & $\begin{aligned} &y_{t} = l_{t-1}(1 + \epsilon_t) \\
    ...
    \end{aligned}$\\
\hline
Trend="A" & $\begin{aligned} &y_{t} = (l_{t-1} + b_{t-1})(1 + \epsilon_t) \\
    ...
    \end{aligned}$\\
\hline
Trend="Ad" & $\begin{aligned} &y_{t} = (l_{t-1} + \phi b_{t-1})(1 + \epsilon_t) \\
    ...
    \end{aligned}$\\
\hline
Trend="M" & $\begin{aligned} &y_{t} = l_{t-1} b_{t-1} (1 + \epsilon_t) \\
      &l_t = l_{t-1} b_{t-1} (1 + \alpha \epsilon_t) \\
    ...
    \end{aligned}$\\
\hline
Trend="Md" & $\begin{aligned} &y_{t} = l_{t-1} b_{t-1}^\phi (1 + \epsilon_t) \\
...
## 
## Loss function type: likelihood; Loss function value: 521.7742
## Error standard deviation: 30.7654
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1049.548 1049.779 1057.595 1058.135 
## 
## 95% parametric prediction interval was constructed
## 67% of values are in the prediction interval
## Forecast errors:
## MPE: -6.7%; sCE: -79%; Asymmetry: -100%; MAPE: 6.7%
## MASE: 2.208; sMAE: 6.6%; sMSE: 0.5%; rMAE: 0.923; rRMSE: 0.934
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 633.9421
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1273.884 1274.115 1281.931 1282.471 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 3.1%; sCE: 37%; Asymmetry: 58%; MAPE: 7%
## MASE: 0.632; sMAE: 6%; sMSE: 0.5%; rMAE: 0.978; rRMSE: 0.982
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.309614788  15.582896128 484.045402119  -0.015714694   0.033875854 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.242732253   0.059365127   0.007025101   0.647023579   0.702187586 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.205038107   0.213650925   0.074608170  -0.210696760   2.085235457
\end{verbatim}
...
## 141 257.6524           254.9177            260.4029
## 142 257.7071           253.3921            262.0306
## 143 257.7588           251.9346            263.6195
## 144 257.8025           250.4578            265.2442
## 145 257.8508           249.0162            266.8121
## 146 257.9050           247.4699            268.4450
## 147 257.9498           245.8292            270.1204
## 148 257.9818           244.2234            271.6833
## 149 258.0340           242.6535            273.5568
## 150 258.0930           241.0281            275.3041
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.020  0.001  0.003  0.020 -0.024
## beta   0.001  0.006  0.000 -0.008  0.015
## phi    0.003  0.000  0.010  0.046 -0.048
## level  0.020 -0.008  0.046  0.539 -0.754
## trend -0.024  0.015 -0.048 -0.754  1.142
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 20 units, while the upper bound of the interval was approximately 37. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
