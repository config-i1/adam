...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 79.2 and 121.92. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10513413 0.08660610 0.07951365
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10430734 0.08654481 0.08018423
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 523.2989
## Error standard deviation: 31.2028
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1052.598 1052.829 1060.644 1061.184 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 2.1%; sCE: 27%; Asymmetry: 82.5%; MAPE: 2.5%
## MASE: 0.863; sMAE: 2.6%; sMSE: 0.1%; rMAE: 1.02; rRMSE: 1.144
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 674.8036
## Error standard deviation: 0.1097
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1355.607 1355.838 1363.653 1364.194 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -0.8%; sCE: 2.6%; Asymmetry: -9.3%; MAPE: 8.3%
## MASE: 0.981; sMAE: 10.8%; sMSE: 1.7%; rMAE: 0.716; rRMSE: 0.701
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.440379503  15.504629876 479.442538160  -0.015950820   0.033762781 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.248710241   0.059066961   0.006958298   0.643773855   0.698841005 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204008288   0.212632678   0.076445614  -0.193161448   2.092843500
\end{verbatim}
...
## alpha      0.776 0.775 0.888 0.909 0.018 0.664
## beta       0.000 0.000 0.019 0.017 0.004 0.001
## gamma      0.000 0.000 0.000 0.008 0.008 0.001
\end{verbatim}

What we observe in this case is that LASSO has the lowest smoothing parameter \(\alpha\) because it is shrunk directly in the model. The reason why \(\beta\) and \(\gamma\) in LASSO is higher than in other cases is because of the default parameters restriction in ETS (see Section \ref{ETSParametersBounds}). Likelihood and MSE give similar values. They both rely on squared errors, but not the same because the likelihood of Inverse Gaussian distribution also has additional elements.
...
\begin{verbatim}
##     likelihood     MSE     MAE     HAM   LASSO    Huber
## ME      12.021   9.347   5.048   5.356   8.976   47.426
## MAE     22.792  20.698  17.059  17.389  19.947   51.732
## MSE    752.472 680.667 524.190 551.972 645.646 3552.266
\end{verbatim}
...
## 141 257.6609           254.8840            260.4546
## 142 257.7277           253.3515            262.0990
## 143 257.7891           251.8828            263.6976
## 144 257.8503           250.4425            265.2864
## 145 257.9000           248.8902            266.9627
## 146 257.9475           247.2682            268.5523
## 147 257.9997           245.7736            270.1628
## 148 258.0581           244.1677            271.9048
## 149 258.1117           242.5535            273.6633
## 150 258.1577           240.8572            275.5345
\end{verbatim}
...
\begin{verbatim}
##        alpha  beta    phi  level  trend
## alpha  0.037 0.006  0.008  0.036 -0.041
## beta   0.006 0.008  0.003  0.008  0.004
## phi    0.008 0.003  0.012  0.056 -0.057
## level  0.036 0.008  0.056  0.523 -0.707
## trend -0.041 0.004 -0.057 -0.707  1.052
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 16 units, while the upper bound of the interval was approximately 35. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
