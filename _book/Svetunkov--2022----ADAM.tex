...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 79.99 and 118.66. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05349040 0.06365977 0.06525339
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.06453198 0.07652894 0.07420150
\end{verbatim}
...
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DecValTok{1}\NormalTok{,}
...
\CommentTok{# SMA(10)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y, }\DataTypeTok{order=}\DecValTok{10}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\CommentTok{# SMA(20)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y, }\DataTypeTok{order=}\DecValTok{20}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\CommentTok{# SMA(110)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y, }\DataTypeTok{order=}\DecValTok{110}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{msarima}\NormalTok{(y, }\DataTypeTok{orders=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{i=}\DecValTok{1}\NormalTok{), }\DataTypeTok{lags=}\DecValTok{4}\NormalTok{,}
                     \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## 
## Loss function type: likelihood; Loss function value: 526.5983
## Error standard deviation: 32.1707
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1059.197 1059.427 1067.243 1067.783 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 1.2%; sCE: 15.9%; Asymmetry: 63.6%; MAPE: 2.8%
## MASE: 0.794; sMAE: 2.8%; sMSE: 0.1%; rMAE: 0.997; rRMSE: 0.998
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ourModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\StringTok{"ANN"}\NormalTok{, }\DataTypeTok{h=}\DecValTok{12}\NormalTok{, }\DataTypeTok{interval=}\OtherTok{TRUE}\NormalTok{,}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 662.9472
## Error standard deviation: 0.0999
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1331.894 1332.125 1339.941 1340.481 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 2.1%; sCE: 38%; Asymmetry: 34.3%; MAPE: 7.3%
## MASE: 0.825; sMAE: 8.2%; sMSE: 1.1%; rMAE: 0.582; rRMSE: 0.623
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{esModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y, }\DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{silent=}\OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{esModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{silent=}\OtherTok{FALSE}\NormalTok{, }\DataTypeTok{persistence=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.912334486  15.457311574 481.401644741  -0.016918438   0.033747409 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.270285948   0.058886696   0.006986731   0.641809134   0.700267358 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.203385679   0.213066667   0.083077300  -0.250560170   2.230998209
\end{verbatim}
...
## 141 257.6610           254.9074            260.4087
## 142 257.7227           253.2990            262.1045
## 143 257.7821           251.8600            263.7838
## 144 257.8435           250.3853            265.3168
## 145 257.9162           248.8641            266.9524
## 146 257.9844           247.3792            268.6211
## 147 258.0385           245.8578            270.2522
## 148 258.0914           244.2809            271.9703
## 149 258.1436           242.6693            273.6761
## 150 258.1962           241.0971            275.4271
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.005 -0.003 -0.001  0.002 -0.007
## beta  -0.003  0.006  0.000 -0.012  0.026
## phi   -0.001  0.000  0.010  0.043 -0.048
## level  0.002 -0.012  0.043  0.456 -0.646
## trend -0.007  0.026 -0.048 -0.646  0.986
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 23 units, while the upper bound of the interval was approximately 37. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
...
\chapter{adam() in R Cheat Sheet}\label{cheatSheet}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ourModel <-}\StringTok{ }\KeywordTok{auto.adam}\NormalTok{(y, }\DataTypeTok{model=}\StringTok{"AAN"}\NormalTok{, }\DataTypeTok{distribution=}\StringTok{"dgnorm"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
