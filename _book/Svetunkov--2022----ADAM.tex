...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 81.37 and 119.85. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05783387 0.05432064 0.05548785
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.06952832 0.06083809 0.05483217
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 520.5049
## Error standard deviation: 30.4059
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1047.010 1047.241 1055.056 1055.596 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -0.8%; sCE: -8%; Asymmetry: -31.4%; MAPE: 2.9%
## MASE: 0.879; sMAE: 2.8%; sMSE: 0.1%; rMAE: 0.824; rRMSE: 0.867
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 616.2122
## Error standard deviation: 0.096
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1238.424 1238.655 1246.471 1247.011 
## 
## 95% parametric prediction interval was constructed
## 50% of values are in the prediction interval
## Forecast errors:
## MPE: 16.9%; sCE: 228.8%; Asymmetry: 100%; MAPE: 16.9%
## MASE: 2.043; sMAE: 19.1%; sMSE: 5.1%; rMAE: 0.778; rRMSE: 0.825
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.679135023  15.471995095 482.323651931  -0.016476858   0.033711126 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.259625094   0.058942635   0.007000112   0.642418814   0.700937633 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.203578883   0.213270608   0.079800492  -0.238568437   2.183892257
\end{verbatim}
...
\begin{equation}
  \begin{aligned}
    &{y}_{t} = \exp \left( \mathbf{w}_{ETS}^\prime \log \mathbf{v}_{ETS,t-\mathbf{l}} + \mathbf{w}_{ARIMA}^\prime \log \mathbf{v}_{ARIMA,t-\mathbf{l}} + \log(1+\epsilon_t) \right) \\
        &\log \mathbf{v}_{ETS,t} = \mathbf{F}_{ETS} \log \mathbf{v}_{ETS,t-\mathbf{l}} + \log(\mathbf{1}_k + \mathbf{g}_ETS \epsilon_t) \\
    &\log \mathbf{v}_{ARIMA,t} = \mathbf{F}_{ARIMA} \log \mathbf{v}_{ARIMA,t-\mathbf{l}} + \mathbf{g}_ARIMA \log(1+\epsilon_t)
  \end{aligned} ,
  \label{eq:ADAMETSARIMA}
\end{equation}
where the subscript reflects, which part corresponds to which model.
...
## 141 257.6557           254.9003            260.3934
## 142 257.7120           253.3486            262.0895
## 143 257.7705           251.8429            263.7293
## 144 257.8201           250.3982            265.3047
## 145 257.8722           248.8650            266.9762
## 146 257.9167           247.2704            268.6585
## 147 257.9597           245.6950            270.3988
## 148 258.0131           244.1396            272.1792
## 149 258.0672           242.6168            273.8897
## 150 258.1169           240.8426            275.6225
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.011 -0.001  0.001  0.009 -0.012
## beta  -0.001  0.007  0.000 -0.012  0.020
## phi    0.001  0.000  0.013  0.061 -0.063
## level  0.009 -0.012  0.061  0.560 -0.743
## trend -0.012  0.020 -0.063 -0.743  1.063
\end{verbatim}
