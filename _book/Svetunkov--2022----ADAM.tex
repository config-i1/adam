...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 78.52 and 119.79. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05759620 0.04000023 0.03949837
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05610040 0.04060049 0.03983313
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 523.4697
## Error standard deviation: 31.2522
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1052.939 1053.170 1060.986 1061.526 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -1.8%; sCE: -20.4%; Asymmetry: -79.6%; MAPE: 2.3%
## MASE: 0.797; sMAE: 2.2%; sMSE: 0.1%; rMAE: 0.617; rRMSE: 0.639
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 700.2735
## Error standard deviation: 0.1243
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1406.547 1406.778 1414.593 1415.134 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 2.1%; sCE: 42.8%; Asymmetry: 39.7%; MAPE: 8.6%
## MASE: 0.925; sMAE: 10.5%; sMSE: 1.4%; rMAE: 1.05; rRMSE: 1.045
\end{verbatim}
...
\tightlist
\item
  Inverse Gaussian: \(\left(1+\epsilon_t \right) \sim \mathcal{IG}(1, s)\), so that \(y_t = \mu_t (1+\epsilon_t) \sim \mathcal{IG}(\mu_t, s)\);
\item
  Gamma: \(\left(1+\epsilon_t \right) \sim \Gamma (s^{-1}, s)\), so that \(y_t = \mu_t (1+\epsilon_t) \sim \Gamma (s^{-1}, s \mu_t)\);
\item
  Log Normal: \(\left(1+\epsilon_t \right) \sim \mathrm{log}\mathcal{N}\left(-\frac{\sigma^2}{2}, \sigma^2\right)\) so that \(y_t = \mu_t (1+\epsilon_t) \sim \mathrm{log}\mathcal{N}(\log \mu_t -\frac{\sigma^2}{2}, \sigma^2)\).
\end{enumerate}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.673957044  15.537660657 480.559843766  -0.016437650   0.033880610 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.259388379   0.059192797   0.006974514   0.645145339   0.699654830 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204442903   0.212880296   0.079727734  -0.222319738   2.120381379
\end{verbatim}
...
## 141 257.6602           254.8943            260.4167
## 142 257.7215           253.3850            262.0837
## 143 257.7849           251.9121            263.6596
## 144 257.8388           250.4519            265.3532
## 145 257.8961           248.9632            266.9818
## 146 257.9327           247.4110            268.6014
## 147 257.9896           245.8268            270.3340
## 148 258.0566           244.2705            272.1079
## 149 258.1270           242.7828            273.9067
## 150 258.1867           241.1878            275.5850
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.011 -0.001  0.000  0.007 -0.014
## beta  -0.001  0.006  0.000 -0.013  0.025
## phi    0.000  0.000  0.011  0.049 -0.052
## level  0.007 -0.013  0.049  0.513 -0.704
## trend -0.014  0.025 -0.052 -0.704  1.037
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 25 units, while the upper bound of the interval was approximately 94. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
