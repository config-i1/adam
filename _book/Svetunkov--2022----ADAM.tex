...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 79.96 and 119.77. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.09215618 0.08635310 0.10428109
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.09253531 0.08607823 0.09742656
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 524.0661
...
##      AIC     AICc      BIC     BICc 
## 1054.132 1054.363 1062.179 1062.719 
## 
## Forecast errors:
## ME: 17.587; MAE: 24.254; RMSE: 29.845
## sCE: 20.525%; Asymmetry: 77.5%; sMAE: 2.359%; sMSE: 0.084%
## MASE: 0.775; RMSSE: 0.759; rMAE: 1.022; rRMSE: 1.016
\end{verbatim}
...
## Distribution assumed in the model: Normal
## Loss function type: likelihood; Loss function value: 667.6886
## Persistence vector g:
##  alpha 
## 0.2211 
## 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1341.377 1341.608 1349.424 1349.964 
## 
## Forecast errors:
## ME: -118.069; MAE: 120.251; RMSE: 142.319
## sCE: -120.067%; Asymmetry: -95.1%; sMAE: 10.19%; sMSE: 1.455%
## MASE: 1.003; RMSSE: 0.919; rMAE: 1.668; rRMSE: 1.679
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.130039357  15.526680928 476.669779192  -0.015307295   0.033733256 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.234522853   0.059150968   0.006918056   0.644689446   0.696817272 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204298433   0.212016928   0.072084862  -0.177058547   1.995739950
\end{verbatim}
...
## 141 257.6577           254.8731            260.4546
## 142 257.7126           253.3324            262.1466
## 143 257.7583           251.8481            263.6208
## 144 257.8063           250.3090            265.2870
## 145 257.8538           248.8263            266.9508
## 146 257.8912           247.2975            268.5922
## 147 257.9341           245.7038            270.3216
## 148 257.9794           244.0430            272.1234
## 149 258.0243           242.5485            273.8785
## 150 258.0600           240.7627            275.6090
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.005 -0.002 -0.002 -0.010  0.006
## beta  -0.002  0.005  0.000 -0.011  0.022
## phi   -0.002  0.000  0.012  0.060 -0.064
## level -0.010 -0.011  0.060  0.574 -0.754
## trend  0.006  0.022 -0.064 -0.754  1.076
\end{verbatim}
...
A thing to note is that in the case of multiplicative trend or multiplicative seasonality, it makes sense to take trimmed mean instead of the basic arithmetic one on the step (5). This is because the models with these components might exhibit explosive behaviour, and thus the expectation might become unrealistic. I suggest using 1\% trimming, although this does not have any scientific merit and is only based on my personal expertise.
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 13 units, while the upper bound of the interval was approximately 32. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
