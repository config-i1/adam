...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 82.13 and 120.73. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05942542 0.07538273 0.09966768
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.05976077 0.07532114 0.09951711
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 531.0349
## Error standard deviation: 33.5198
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1068.070 1068.301 1076.116 1076.657 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 1.7%; sCE: 21.8%; Asymmetry: 56.8%; MAPE: 3.5%
## MASE: 0.894; sMAE: 3.4%; sMSE: 0.2%; rMAE: 1.121; rRMSE: 1.097
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 659.538
## Error standard deviation: 0.1071
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1325.076 1325.307 1333.122 1333.663 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -0.4%; sCE: 4.7%; Asymmetry: 11.7%; MAPE: 7.2%
## MASE: 0.926; sMAE: 9.1%; sMSE: 1.1%; rMAE: 1.007; rRMSE: 0.949
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.573086776  15.514315506 481.565291533  -0.016266632   0.033790922 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.254777034   0.059103860   0.006989106   0.644176016   0.700386372 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204135730   0.213102879   0.078310353  -0.224404517   2.118243375
\end{verbatim}
...
\begin{verbatim}
## Warning in matrix(distributionsTable, 8, 3, byrow = TRUE, dimnames = list(c("\
## \textbf{Normal}", : data length [21] is not a sub-multiple or multiple of the
## number of rows [8]
\end{verbatim}
...
\hline
\textbf{Asymmetric Laplace} & $1+\frac{\epsilon_t}{\mu_{y,t}} \sim \mathcal{IG}(1, \sigma^2)$ & $\begin{aligned} &-\frac{T}{2} \log \left(2 \pi \sigma^2 \right) -\frac{1}{2} \sum_{t=1}^T \left( \left(\frac{y_t}{\mu_{y,t}}\right)^3 \right) -\\
...
\hline
\textbf{Inverse Gaussian} & $1+\frac{\epsilon_t}{\mu_{y,t}} \sim \mathcal{\Gamma}(\sigma^{-2}, \sigma^2)$ & $\begin{aligned} &-T \log \Gamma \left(\sigma^{-2}\right) -\frac{T}{\sigma^2} \log \sigma^2 + \\
...
\hline
\textbf{Gamma} & $1+\frac{\epsilon_t}{\mu_{y,t}} \sim \mathrm{log}\mathcal{N}\left(-\frac{\sigma^2}{2}, \sigma^2\right)$ & $\begin{aligned} &-\frac{T}{2} \log \left(2 \pi \sigma^2\right) -\\
...
\hline
\textbf{Log Normal} & $\epsilon_t \sim \mathcal{N}(0, \sigma^2)$ & $-\frac{T}{2} \log(2 \pi \sigma^2) -\frac{1}{2} \sum_{t=1}^T \frac{\epsilon_t^2}{\sigma^2}$ & $\hat{\sigma}^2 = \frac{1}{T} \sum_{t=1}^T e_t^2$\\
\hline
\end{tabular}
\end{table}
...
\hline
\textbf{Asymmetric Laplace} & $1+\epsilon_t \sim \mathcal{IG}(1, \sigma^2)$ & $\begin{aligned} &-\frac{T}{2} \log \left(2 \pi \sigma^2 \right) -\frac{1}{2}\sum_{t=1}^T \left(1+\epsilon_{t}\right)^3 -\\
...
\hline
\textbf{Inverse Gaussian} & $1+\epsilon_t \sim \mathcal{\Gamma}(\sigma^{-2}, \sigma)$ & $\begin{aligned} &-T \log \Gamma \left(\sigma^{-2}\right) -\frac{T}{\sigma^2} \log \sigma^2 + \\
...
\hline
\textbf{Gamma} & $1+\epsilon_t \sim \mathrm{log}\mathcal{N}\left(-\frac{\sigma^2}{2}, \sigma^2\right)$ & $\begin{aligned} &-\frac{T}{2} \log \left(2 \pi \sigma^2\right) -\\
...
\hline
\textbf{Log Normal} & $\epsilon_t \sim \mathcal{N}(0, \sigma^2)$ & $\begin{aligned} &-\frac{T}{2} \log(2 \pi \sigma^2) -\frac{1}{2} \sum_{t=1}^T \frac{\epsilon_t^2}{\sigma^2} -\\
                                         &\sum_{t=1}^T \log |\mu_{y,t}|\end{aligned}$ & $\hat{\sigma}^2 = \frac{1}{T} \sum_{t=1}^T e_t^2$\\
\hline
\end{tabular}
\end{table}
...
Given that the scale of the original variable is now removed in the standardised residuals, it might be easier to spot the non-linearity. In our case, in Figure \ref{fig:adamModelSeat03Resid}, it is still not apparent, but there is a slight U-shape in the LOWESS line and a slight change in the variance. Another plot that we have already used before is standardised residuals over time (Figure \ref{fig:adamModelSeat03ResidTime}):
...
The plot in Figure \ref{fig:adamModelSeat03ResidTime} shows a slight decline in the residuals around 1977. Still, there is no apparent non-linearity in the residuals, so it is not clear whether any transformations are needed or not.
...
We see in Figure \ref{fig:adamModelSeat05ResidTime} that there is no specific pattern in the outliers, they happen randomly, so they appear not because of the omitted variables or wrong transformations. We have nine observations lying outside the bounds, which given that the sample size of 192 observations, means that the 95\% interval contains \(\frac{192-9}{192} \times 100 \mathrm{\%} \approx 95.3\mathrm{\%}\) of observations, which is close to the nominal value.
...
In Figure \ref{fig:adamModelSeat05Outliers}, among all these points, there is one special that happens on observation 170. This was when the law for seatbelts was introduced, and the model cannot correctly capture the change in injuries and deaths. This observation can be considered as an outlier because of that and should be treated separately. It should not be removed, but the model needs to take it into account.
...
Comparing the two values above, I would conclude that adding dummies improves the model. This could be a mistake, given that we do not know the reasons behind most of them. So, instead of including all of them, we could try the model with the outlier for the suspicious observation 170, which corresponds to the seventh outlier:
...
This model is slightly worse than the one with all outliers in terms of AICc, but this could be for purely technical reasons. If we cannot motivate or predict them, we should not include them as dummy variables. Model 7 with higher AICc looks more reasonable than the previous models from this point of view. A similar diagnostics plot can be done for the new model (Figure \ref{fig:adamModelSeat07Outliers}):
...
We could continue the exploration by introducing other dummy variables based on the plot in Figure \ref{fig:adamModelSeat07Outliers}. However, in general, we should not do that unless we have a good reason for adding more variables (e.g.~we know that the model did not capture something that happened).
...
If a variable would still cause an issue with it, it would make sense to construct a model for the variance (e.g.~GARCH) to address the problem and improve the model's performance.
...
As the plot in Figure \ref{fig:adamModelSeat05Multistep} demonstrates, the mean of the residuals increases with the increase of the horizon, thus implying that some information has not been taken into account by the model.
...
According to the QQ plot in Figure \ref{fig:adamModelSeat17QQ}, the residuals of the new model are much closer to the theoretical ones. Only the right tail has a slight deviation from normality -- the actual values are a bit further away than expected. This can be addressed by using a skewed distribution, for example, Inverse Gaussian:
...
\caption{\label{fig:adamModelSeat18QQ}QQ plot of residuals extracted from the multiplicative model with Inverse Gaussian distribution.}
\end{figure}

The QQ plot in Figure \ref{fig:adamModelSeat18QQ} demonstrates that the empirical residuals follow the assumed distribution much closer than in the previous cases: just a few observations lie slightly away from the line, but they could happen at random. So, based on this simple analysis, we could conclude that the Inverse Gaussian distribution is more suitable for this situation than the Normal one.
...
## 141 257.6643           254.9090            260.4200
## 142 257.7177           253.4074            262.0380
## 143 257.7701           251.9360            263.6042
## 144 257.8239           250.5090            265.1912
## 145 257.8775           248.9786            266.8460
## 146 257.9309           247.4406            268.4769
## 147 257.9775           245.8465            270.1965
## 148 258.0200           244.1659            271.9313
## 149 258.0641           242.6189            273.6509
## 150 258.1087           240.9723            275.4383
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.031  0.005  0.005  0.037 -0.048
## beta   0.005  0.012  0.002 -0.002  0.014
## phi    0.005  0.002  0.014  0.061 -0.065
## level  0.037 -0.002  0.061  0.520 -0.692
## trend -0.048  0.014 -0.065 -0.692  1.003
\end{verbatim}
