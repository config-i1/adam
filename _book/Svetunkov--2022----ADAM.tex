...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (\(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 79.84 and 119.89. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.07970565 0.09877186 0.10798484
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.06834179 0.09424684 0.10610326
\end{verbatim}
...
## 
## Loss function type: likelihood; Loss function value: 532.8464
## Error standard deviation: 34.0868
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1071.693 1071.924 1079.739 1080.280 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 1.1%; sCE: 14.7%; Asymmetry: 35.1%; MAPE: 2.8%
## MASE: 0.786; sMAE: 2.8%; sMSE: 0.1%; rMAE: 0.898; rRMSE: 1.038
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 695.3001
## Error standard deviation: 0.1169
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1396.600 1396.831 1404.647 1405.187 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: -5.3%; sCE: -77%; Asymmetry: -55.1%; MAPE: 9.2%
## MASE: 1.208; sMAE: 12.9%; sMSE: 2.2%; rMAE: 1.233; rRMSE: 1.095
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.565486410  15.058880238 454.697966764  -0.016120407   0.032875773 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.254429579   0.057368818   0.006599172   0.625265709   0.680568124 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.198143161   0.207072885   0.078203556  -0.227975404   2.011663106
\end{verbatim}
...
Based on that, we can conclude that the introduction of the law reduced on average the number of incidents by approximately 24\%, while the increase of the petrol price by 1\% leads on average to decrease in the number of incidents by 0.28\%. Finally, the distance negatively impacts the incidents as well, reducing it on average by 0.1\% for each 1\% increase in the distance. This is the standard interpretation of parameters, which we can use based on the estimated model \citep[see, for example, discussion in Section 8.3 of][]{SvetunkovSBA}. We will discuss how to do the analysis using ADAM in Chapter \ref{ADAMUncertainty}, introducing the standard errors and confidence intervals for the parameters.
...
\begin{verbatim}
##  [1]  14  33  59  61  66  86  92 109 156 177
\end{verbatim}
...
\hypertarget{connection-with-arch-and-garch}{%
\section{Connection with ARCH and GARCH}\label{connection-with-arch-and-garch}}

Scale model is not a new invention. Engle (1982) proposed an Autoregressive Conditional Heteroscedasticity (ARCH) model to capture the time varying variance using lagged values of the squared error term. Bollerslev (1986) expanded the idea by also including lagged values of variance, introducing a Generalised ARCH (GARCH), which can be formulated as:
\begin{equation}
    \sigma_t^2 = a_0 + a_1 \sigma_{t-1}^2 + \dots + a_p \sigma_{t-p}^2 + b_1 \epsilon_{t-1}^2 + \dots + b_q \epsilon_{t-q}^2 ,
    \label{eq:GARCH}
\end{equation}
where \(\epsilon_t\sim \mathcal{N}(0,\sigma_t^2)\) and \(a_j\) and \(b_j\) are the parameters of the model, which all lie between zero and one to guarantee that the resulting value is positive. To make sure that the produced values are always positive, \href{https://doi.org/10.1080/07474938608800097}{Geweke (1986)} suggested to build GARCH in logarithms, leading to the model:
\begin{equation}
    \log \sigma_t^2 = a_0 + a_1 \log \sigma_{t-1}^2 + \dots + a_p \log \sigma_{t-p}^2 + b_1 \log \epsilon_{t-1}^2 + \dots + b_q \log \epsilon_{t-q}^2 .
    \label{eq:logGARCH}
\end{equation}
Psaradakis (1999) pointed out that the model \eqref{eq:logGARCH} is equivalent to ARMA(p,q) applied to logarithms of squared error \(\epsilon_t^2\). In our notations it can be written as:
\begin{equation}
    \log \epsilon_t^2 = a_0 + b_1^\prime \log \epsilon_{t-1}^2 + \dots + b_q^\prime \log \epsilon_{t-q}^2 + a_1^\prime \log \eta_{t-1}^2 + \dots + a_p^\prime \log \eta_{t-p}^2 + \log \eta_t^2,
    \label{eq:logGARCHARMA}
\end{equation}
which can be obtained by substituting \(\log \sigma_{t-j}^2 = \log \epsilon_{t-j}^2 - \log \eta_{t-j}^2\) for all \(j\). Given the connection of logARMA with logGARCH and the discussion in Subsection \ref{ADAMARIMAPureMultiplicative}, the model \eqref{eq:logGARCHARMA} is just a special case of the scale model for ADAM, being a special case of \eqref{eq:ADAMSMGeneral}. ADAM Scale Model not only supports the ARMA elements, but it also allows for explicit time series components modelling in the variance and the usage of explanatory variables.
...
Furthermore, it can be shown that ETS(M,N,N) applied to the \(\epsilon_t^2\) can be considered as a special case of GARCH(1,1) with the restriction on parameter \(a_1=1-b_1\), \(b_1= \alpha\) and \(a_0=0\), because the latter becomes equivalent to SES:
\begin{equation}
...
Coming back to the original GARCH model, Bollerslev (1986) argue that GARCH, being equivalent to ARMA, will be a stationary process if its parameters are restricted so that \(\sum_{j=1}^p a_j + \sum_{j=1}^q b_j < 0\) and \(a_j, b_j \in [0,1)\) for all \(j\). For ADAM ARIMA, this means that the model can also be applied to the data without logarithmic transform as long as these restrictions are satisfied.
...
## 141 257.6612           254.9506            260.4015
## 142 257.7259           253.4025            262.0689
## 143 257.7978           251.9531            263.6945
## 144 257.8672           250.5062            265.3427
## 145 257.9232           249.0430            266.8684
## 146 257.9821           247.5568            268.5193
## 147 258.0283           245.9772            270.1333
## 148 258.0851           244.4968            271.7834
## 149 258.1402           242.8664            273.5494
## 150 258.1894           241.1918            275.2054
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.027 -0.005  0.006  0.025  0.008
## beta  -0.005  0.012 -0.007 -0.037 -0.035
## phi    0.006 -0.007  0.013  0.090  0.016
## level  0.025 -0.037  0.090  1.253 -0.208
## trend  0.008 -0.035  0.016 -0.208  0.267
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 17 units, while the upper bound of the interval was approximately 51. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
