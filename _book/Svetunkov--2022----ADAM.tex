...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 82.3 and 116.84. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\NormalTok{model1 <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\NormalTok{model2 <-}\StringTok{ }\KeywordTok{ces}\NormalTok{(y,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
...
\NormalTok{y <-}\StringTok{ }\KeywordTok{rnorm}\NormalTok{(}\DecValTok{100}\NormalTok{,}\DecValTok{100}\NormalTok{,}\DecValTok{10}\NormalTok{)}
\NormalTok{test <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
...
\NormalTok{errorMeasures <-}\StringTok{ }\KeywordTok{array}\NormalTok{(}\OtherTok{NA}\NormalTok{, }\KeywordTok{c}\NormalTok{(nsim,}\DecValTok{2}\NormalTok{,}\KeywordTok{length}\NormalTok{(test}\OperatorTok{$}\NormalTok{accuracy)),}
...
\NormalTok{  testModel1 <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y,}\StringTok{"ANN"}\NormalTok{,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\NormalTok{  errorMeasures[i,}\DecValTok{1}\NormalTok{,] <-}\StringTok{ }\KeywordTok{measures}\NormalTok{(testModel1}\OperatorTok{$}\NormalTok{holdout,}
\NormalTok{                                  testModel1}\OperatorTok{$}\NormalTok{forecast,}
                                  \KeywordTok{actuals}\NormalTok{(testModel1))}
  \CommentTok{# Apply CES}
\NormalTok{  testModel2 <-}\StringTok{ }\KeywordTok{ces}\NormalTok{(y,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\NormalTok{  errorMeasures[i,}\DecValTok{2}\NormalTok{,] <-}\StringTok{ }\KeywordTok{measures}\NormalTok{(testModel2}\OperatorTok{$}\NormalTok{holdout,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{model1Forecast <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(model1,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{interval=}\StringTok{"p"}\NormalTok{,}\DataTypeTok{level=}\FloatTok{0.95}\NormalTok{)}
\NormalTok{model2Forecast <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(model2,}\DataTypeTok{h=}\DecValTok{10}\NormalTok{,}\DataTypeTok{interval=}\StringTok{"p"}\NormalTok{,}\DataTypeTok{level=}\FloatTok{0.95}\NormalTok{)}
...
\begin{Highlighting}[]
\CommentTok{# Coverage}
\KeywordTok{setNames}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\KeywordTok{mean}\NormalTok{(model1}\OperatorTok{$}\NormalTok{holdout }\OperatorTok{>}\StringTok{ }\NormalTok{model1Forecast}\OperatorTok{$}\NormalTok{lower }\OperatorTok{&}
\StringTok{                }\NormalTok{model1}\OperatorTok{$}\NormalTok{holdout }\OperatorTok{<}\StringTok{ }\NormalTok{model1Forecast}\OperatorTok{$}\NormalTok{upper),}
           \KeywordTok{mean}\NormalTok{(model2}\OperatorTok{$}\NormalTok{holdout }\OperatorTok{>}\StringTok{ }\NormalTok{model2Forecast}\OperatorTok{$}\NormalTok{lower }\OperatorTok{&}
\StringTok{                }\NormalTok{model2}\OperatorTok{$}\NormalTok{holdout }\OperatorTok{<}\StringTok{ }\NormalTok{model2Forecast}\OperatorTok{$}\NormalTok{upper)),}
         \KeywordTok{c}\NormalTok{(}\StringTok{"Model 1"}\NormalTok{, }\StringTok{"Model 2"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obs <-}\StringTok{ }\DecValTok{100}
\NormalTok{roh <-}\StringTok{ }\DecValTok{8}
\NormalTok{h <-}\StringTok{ }\DecValTok{3}
\NormalTok{data <-}\StringTok{ }\NormalTok{y}
\NormalTok{returnedValues1 <-}\StringTok{ }\KeywordTok{setNames}\NormalTok{(}\KeywordTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{,}\DecValTok{3}\NormalTok{),}
                            \KeywordTok{c}\NormalTok{(}\StringTok{"actuals"}\NormalTok{,}\StringTok{"holdout"}\NormalTok{,}\StringTok{"pred"}\NormalTok{))}
\NormalTok{returnedValues1}\OperatorTok{$}\NormalTok{actuals <-}\StringTok{ }\NormalTok{y}
\NormalTok{returnedValues1}\OperatorTok{$}\NormalTok{holdout <-}
\StringTok{  }\NormalTok{returnedValues1}\OperatorTok{$}\NormalTok{pred <-}
...
\NormalTok{  testModel <-}\StringTok{ }\KeywordTok{arima}\NormalTok{(}\DataTypeTok{x=}\NormalTok{data[}\DecValTok{1}\OperatorTok{:}\NormalTok{(obs}\OperatorTok{-}\NormalTok{roh}\OperatorTok{+}\NormalTok{i}\OperatorTok{-}\NormalTok{h)],}\DataTypeTok{order=}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{))}
\NormalTok{  returnedValues1}\OperatorTok{$}\NormalTok{holdout[,i] <-}\StringTok{ }\NormalTok{data[}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{(obs}\OperatorTok{-}\NormalTok{roh}\OperatorTok{+}\NormalTok{i}\OperatorTok{-}\NormalTok{h))]}
\NormalTok{  returnedValues1}\OperatorTok{$}\NormalTok{pred[,i] <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(testModel, }\DataTypeTok{n.ahead=}\NormalTok{h)}\OperatorTok{$}\NormalTok{pred}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.09458537 0.11680134 0.10122718
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ourCall <-}\StringTok{ "predict(arima(x=data,order=c(0,2,2)),n.ahead=h)"}
\NormalTok{returnedValues2 <-}\StringTok{ }\KeywordTok{ro}\NormalTok{(y, }\DataTypeTok{h=}\DecValTok{3}\NormalTok{, }\DataTypeTok{origins=}\DecValTok{8}\NormalTok{,}
                      \DataTypeTok{call=}\NormalTok{ourCall, }\DataTypeTok{value=}\NormalTok{ourValue)}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10093419 0.12124729 0.09761815
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{aeValuesh1 <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\KeywordTok{abs}\NormalTok{(returnedValues1}\OperatorTok{$}\NormalTok{holdout }\OperatorTok{-}
...
\KeywordTok{colnames}\NormalTok{(aeValuesh1) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"ARIMA(0,1,1)"}\NormalTok{,}\StringTok{"ARIMA(0,2,2)"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y <-}\StringTok{ }\KeywordTok{sim.sma}\NormalTok{(}\DecValTok{10}\NormalTok{,}\DecValTok{120}\NormalTok{)}
...
\CommentTok{# SMA(1)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{order=}\DecValTok{1}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
...
\CommentTok{# SMA(10)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{order=}\DecValTok{10}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\CommentTok{# SMA(20)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{order=}\DecValTok{20}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\CommentTok{# SMA(110)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{sma}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{order=}\DecValTok{110}\NormalTok{,}
                 \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\NormalTok{testModel}\OperatorTok{$}\NormalTok{model)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y <-}\StringTok{ }\KeywordTok{sim.ssarima}\NormalTok{(}\DataTypeTok{orders=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{i=}\DecValTok{1}\NormalTok{), }\DataTypeTok{lags=}\DecValTok{4}\NormalTok{,}
                 \DataTypeTok{obs=}\DecValTok{120}\NormalTok{, }\DataTypeTok{sd=}\DecValTok{50}\NormalTok{)}
\NormalTok{testModel <-}\StringTok{ }\KeywordTok{msarima}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{orders=}\KeywordTok{list}\NormalTok{(}\DataTypeTok{i=}\DecValTok{1}\NormalTok{), }\DataTypeTok{lags=}\DecValTok{4}\NormalTok{,}
                     \DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{)}
\KeywordTok{plot}\NormalTok{(testModel, }\DecValTok{7}\NormalTok{, }\DataTypeTok{main=}\StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ourModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y, }\DataTypeTok{model=}\StringTok{"ANN"}\NormalTok{, }\DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{loss=}\StringTok{"MSE"}\NormalTok{)}
...
\begin{Highlighting}[]
\CommentTok{# Generate the data}
\NormalTok{y <-}\StringTok{ }\KeywordTok{sim.es}\NormalTok{(}\StringTok{"ANN"}\NormalTok{, }\DecValTok{120}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{12}\NormalTok{, }\DataTypeTok{persistence=}\FloatTok{0.3}\NormalTok{, }\DataTypeTok{initial=}\DecValTok{1000}\NormalTok{)}
\CommentTok{# Apply ETS(A,N,N) model}
\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\StringTok{"ANN"}\NormalTok{, }\DataTypeTok{h=}\DecValTok{12}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{silent=}\OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
## Loss function type: likelihood; Loss function value: 523.6271
...
##      AIC     AICc      BIC     BICc 
## 1053.254 1053.485 1061.301 1061.841 
## 
## Forecast errors:
## ME: 5.481; MAE: 17.654; RMSE: 22.167
## sCE: 6.934%; Asymmetry: 28.6%; sMAE: 1.861%; sMSE: 0.055%
## MASE: 0.591; RMSSE: 0.607; rMAE: 1.024; rRMSE: 1.022
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ourModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\StringTok{"ANN"}\NormalTok{, }\DataTypeTok{h=}\DecValTok{12}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{y <-}\StringTok{ }\KeywordTok{sim.es}\NormalTok{(}\StringTok{"MNN"}\NormalTok{, }\DecValTok{120}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{12}\NormalTok{, }\DataTypeTok{persistence=}\FloatTok{0.3}\NormalTok{, }\DataTypeTok{initial=}\DecValTok{1000}\NormalTok{)}
\NormalTok{ourModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\StringTok{"MNN"}\NormalTok{, }\DataTypeTok{h=}\DecValTok{12}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{,}
...
## Distribution assumed in the model: Normal
## Loss function type: likelihood; Loss function value: 624.42
## Persistence vector g:
##  alpha 
## 0.2357 
## 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1254.840 1255.071 1262.886 1263.427 
## 
## Forecast errors:
## ME: 30.951; MAE: 36.518; RMSE: 48.021
## sCE: 43.579%; Asymmetry: 78.6%; sMAE: 4.285%; sMSE: 0.317%
## MASE: 0.438; RMSSE: 0.475; rMAE: 0.611; rRMSE: 0.695
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{esModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\DataTypeTok{h=}\DecValTok{10}\NormalTok{, }\DataTypeTok{persistence=}\KeywordTok{c}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.1}\NormalTok{))}
\KeywordTok{plot}\NormalTok{(esModel,}\DecValTok{7}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{discountMatrix <-}\StringTok{ }\NormalTok{adamETSBJ}\OperatorTok{$}\NormalTok{transition }\OperatorTok{-}\StringTok{ }\NormalTok{adamETSBJ}\OperatorTok{$}\NormalTok{persistence }\OperatorTok{%*%}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{adamETSAir}\OperatorTok{$}\NormalTok{B[}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{] <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.2}\NormalTok{,}\FloatTok{0.1}\NormalTok{,}\FloatTok{0.3}\NormalTok{)}
\NormalTok{adamETSAir <-}\StringTok{ }\KeywordTok{adam}\NormalTok{(AirPassengers, }\StringTok{"AAA"}\NormalTok{, }\DataTypeTok{lags=}\DecValTok{12}\NormalTok{,}
                   \DataTypeTok{h=}\DecValTok{12}\NormalTok{, }\DataTypeTok{holdout=}\OtherTok{TRUE}\NormalTok{,}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.308179120  15.605151100 483.211001036  -0.015743477   0.033908736 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.242666620   0.059449910   0.007012991   0.647947637   0.701582107 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.205330936   0.213466699   0.074587997  -0.209477006   2.069671408
\end{verbatim}
...
\NormalTok{y <-}\StringTok{ }\KeywordTok{sim.es}\NormalTok{(}\StringTok{"ANN"}\NormalTok{, }\DataTypeTok{obs=}\DecValTok{120}\NormalTok{, }\DataTypeTok{persistence=}\FloatTok{0.2}\NormalTok{)}
\CommentTok{# Estimate ETS(A,N,N)}
\NormalTok{esModel <-}\StringTok{ }\KeywordTok{es}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\StringTok{"ANN"}\NormalTok{)}
\CommentTok{# Estimate ARIMA(0,1,1)}
\NormalTok{ssarimaModel <-}\StringTok{ }\KeywordTok{ssarima}\NormalTok{(y}\OperatorTok{$}\NormalTok{data, }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{), }\DataTypeTok{initial=}\StringTok{"optimal"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Smoothing parameter and theta_1}
...
\NormalTok{SeatbeltsWithOutliers <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(Seatbelts[,}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{4}\NormalTok{,}\DecValTok{7}\NormalTok{)]),}
...
\NormalTok{SeatbeltsWithOutliers}\OperatorTok{$}\NormalTok{drivers <-}\StringTok{ }\KeywordTok{ts}\NormalTok{(SeatbeltsWithOutliers}\OperatorTok{$}\NormalTok{drivers,}
...
\NormalTok{adamSeat06 <-}\StringTok{ }\KeywordTok{adam}\NormalTok{(SeatbeltsWithOutliers, }\StringTok{"MNM"}\NormalTok{, }\DataTypeTok{lags=}\DecValTok{12}\NormalTok{,}
                   \DataTypeTok{formula=}\NormalTok{drivers}\OperatorTok{~}\NormalTok{.)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{etsModel <-}\StringTok{ }\NormalTok{forecast}\OperatorTok{::}\KeywordTok{ets}\NormalTok{(AirPassengers, }\StringTok{"AAA"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{adamPoolBJForecasts <-}\StringTok{ }\KeywordTok{vector}\NormalTok{(}\StringTok{"list"}\NormalTok{,}\DecValTok{3}\NormalTok{)}
\CommentTok{# Produce forecasts from the three models}
\ControlFlowTok{for}\NormalTok{(i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{)\{}
\NormalTok{  adamPoolBJForecasts[[i]] <-}\StringTok{ }\KeywordTok{forecast}\NormalTok{(adamPoolBJ[[i]],}
...
\NormalTok{finalForecast <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\KeywordTok{sapply}\NormalTok{(adamPoolBJForecasts,}
...
\KeywordTok{colnames}\NormalTok{(finalForecast) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Mean"}\NormalTok{, }\StringTok{"Lower bound (2.5%)"}\NormalTok{,}
                             \StringTok{"Upper bound (97.5%)"}\NormalTok{)}
\CommentTok{# Transform the table in the ts format (for convenience)}
\NormalTok{finalForecast <-}\StringTok{ }\KeywordTok{ts}\NormalTok{(finalForecast,}
...
## 141 257.6588           254.8911            260.4172
## 142 257.7223           253.3607            262.1198
## 143 257.7792           251.8657            263.7443
## 144 257.8383           250.4184            265.3995
## 145 257.8878           248.9151            267.0186
## 146 257.9389           247.3994            268.7133
## 147 257.9853           245.8298            270.4355
## 148 258.0363           244.1854            272.1853
## 149 258.0732           242.6039            273.8498
## 150 258.1163           240.8497            275.4874
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.018  0.000  0.000  0.006 -0.013
## beta   0.000  0.008 -0.001 -0.012  0.024
## phi    0.000 -0.001  0.013  0.062 -0.064
## level  0.006 -0.012  0.062  0.539 -0.718
## trend -0.013  0.024 -0.064 -0.718  1.055
\end{verbatim}
