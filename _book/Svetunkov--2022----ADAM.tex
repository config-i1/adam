...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 80.75 and 118.11. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10596749 0.10521159 0.09815714
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.10971388 0.10038650 0.09156143
\end{verbatim}
...
## Loss function type: likelihood; Loss function value: 526.1931
...
##      AIC     AICc      BIC     BICc 
## 1058.386 1058.617 1066.432 1066.973 
## 
## Forecast errors:
## ME: 10.802; MAE: 19.143; RMSE: 28.427
## sCE: 12.834%; Asymmetry: 41.2%; sMAE: 1.895%; sMSE: 0.079%
## MASE: 0.627; RMSSE: 0.761; rMAE: 0.901; rRMSE: 1.079
\end{verbatim}
...
## Distribution assumed in the model: Normal
## Loss function type: likelihood; Loss function value: 641.9193
## Persistence vector g:
##  alpha 
## 0.2394 
## 
...
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1289.839 1290.069 1297.885 1298.425 
## 
## Forecast errors:
## ME: -12.62; MAE: 46.997; RMSE: 56.335
## sCE: -16.335%; Asymmetry: -18.4%; sMAE: 5.069%; sMSE: 0.369%
## MASE: 0.485; RMSSE: 0.486; rMAE: 0.956; rRMSE: 1.025
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.838879225  15.562815124 487.797221224  -0.016829445   0.033939963 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.266927897   0.059288626   0.007079552   0.646189788   0.704903649 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.204773883   0.214477327   0.082045141  -0.246625594   2.220235563
\end{verbatim}
...
There are several important ARIMA models that are often used in practice. We provide their state-space formulations in this subsection.
...
\begin{remark}
The part \(\frac{1}{2} \sum_{t=1}^T \frac{\epsilon_t^2}{\sigma^2}\) does not contain \(\mu_{y,t}\) because when inserted in the formula of Normal distribution, the numerator is: \(y_t - \mu_{y,t}\), which is based on \eqref{eq:ETSADAMNormalDistributionExample02} is \$ \mu\_\{y,t\} \epsilon\_t\$. After inserting in the formula for the Normal distribution that part becomes: \(\frac{1}{2} \sum_{t=1}^T \frac{(\mu_{y,t} \epsilon_t)^2}{\mu_{y,t}^2\sigma^2}\) which after cancellations leads to \eqref{eq:ETSADAMNormalDistributionExample04}.
\end{remark}
...
## 141 257.6581           254.9060            260.4290
## 142 257.7265           253.3724            262.1527
## 143 257.7796           251.8867            263.7278
## 144 257.8316           250.3377            265.3268
## 145 257.8718           248.8294            266.9012
## 146 257.9163           247.2982            268.6126
## 147 257.9676           245.7389            270.3070
## 148 258.0180           244.1858            271.9351
## 149 258.0481           242.5362            273.6875
## 150 258.0883           240.8522            275.4398
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.019  0.001  0.003  0.014 -0.019
## beta   0.001  0.008  0.000 -0.006  0.018
## phi    0.003  0.000  0.012  0.051 -0.054
## level  0.014 -0.006  0.051  0.513 -0.694
## trend -0.019  0.018 -0.054 -0.694  1.021
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 20 units, while the upper bound of the interval was approximately 42. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
