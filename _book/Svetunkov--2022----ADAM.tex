...
Figure \ref{fig:adamExampleNormal} shows an example with a well-behaved demand, for which the best point forecast is the straight line. To capture the uncertainty of demand, we can construct the prediction interval, which will tell which bound the demand will lie in \(1-\alpha\) per cent of cases. The interval in Figure \ref{fig:adamExampleNormal} has the width of 95\% (significance level \(\alpha=0.05\)) and shows that if the situation is repeated many times, the actual demand will be between 79.61 and 122.86. Capturing the uncertainty correctly is important because real-life decisions need to be made based on the full information, not only on the point forecasts.
...
\begin{verbatim}
##         h1         h2         h3 
## 0.08782067 0.08836911 0.08629209
\end{verbatim}
...
\begin{verbatim}
##         h1         h2         h3 
## 0.08797683 0.07437289 0.08895607
\end{verbatim}
...
One of the most powerful and efficient forecasting methods for level time series \citep[which is very popular in practice according to][]{LCFweller2012} is Simple Exponential Smoothing (sometimes also called ``Single Exponential Smoothing''). It was first formulated by \citet{Brown1956} and can be written as:
\begin{equation}
...
Now that we know how time series can be decomposed into components, we can discuss the ETS model, their connection with exponential smoothing methods and focusing on the most popular of those. We do not discuss in detail how the methods were originally derived and how to work with them. Instead, we focus on the main ideas behind the conventional ETS and the connection between Exponential Smoothing and ETS.
...
## 
## Loss function type: likelihood; Loss function value: 531.8528
## Error standard deviation: 33.7746
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1069.706 1069.936 1077.752 1078.292 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 2.2%; sCE: 26.4%; Asymmetry: 85.2%; MAPE: 2.5%
## MASE: 0.834; sMAE: 2.4%; sMSE: 0.1%; rMAE: 0.892; rRMSE: 0.92
\end{verbatim}
...
## Initial values were optimised.
## 
## Loss function type: likelihood; Loss function value: 669.9058
## Error standard deviation: 0.1014
## Sample size: 108
## Number of estimated parameters: 3
## Number of degrees of freedom: 105
## Information criteria:
##      AIC     AICc      BIC     BICc 
## 1345.812 1346.042 1353.858 1354.398 
## 
## 95% parametric prediction interval was constructed
## 100% of values are in the prediction interval
## Forecast errors:
## MPE: 4.4%; sCE: 92.7%; Asymmetry: 69.6%; MAPE: 8.8%
## MASE: 1.514; sMAE: 13%; sMSE: 2.4%; rMAE: 0.837; rRMSE: 0.874
\end{verbatim}
...
\begin{verbatim}
##            ME           MAE           MSE           MPE          MAPE 
##  -5.821814183  15.456275713 481.333449592  -0.016771419   0.033717707 
##           sCE          sMAE          sMSE          MASE         RMSSE 
##  -0.266147758   0.058882750   0.006985741   0.641766123   0.700217757 
##          rMAE         rRMSE          rAME     asymmetry          sPIS 
##   0.203372049   0.213051575   0.081805352  -0.247460468   2.208259836
\end{verbatim}
...
## 141 257.6702           254.8939            260.4878
## 142 257.7339           253.3917            262.1741
## 143 257.7844           251.9358            263.7441
## 144 257.8516           250.4093            265.3653
## 145 257.9017           248.9415            267.0211
## 146 257.9666           247.4321            268.7533
## 147 258.0059           245.9158            270.4343
## 148 258.0622           244.2382            272.2473
## 149 258.1109           242.6339            273.9319
## 150 258.1649           240.9942            275.8210
\end{verbatim}
...
\begin{verbatim}
##        alpha   beta    phi  level  trend
## alpha  0.010 -0.002  0.001  0.005 -0.010
## beta  -0.002  0.005  0.000 -0.009  0.019
## phi    0.001  0.000  0.010  0.042 -0.046
## level  0.005 -0.009  0.042  0.476 -0.669
## trend -0.010  0.019 -0.046 -0.669  1.014
\end{verbatim}
...
What Figure \ref{fig:adamModeliETSCumulative} demonstrates is that for the holdout period of 7 days, the cumulative demand was around 21 units, while the upper bound of the interval was approximately 39. Based on that upper bound, we could place an order (based on what we already have in stock) and have an appropriate safety stock.
