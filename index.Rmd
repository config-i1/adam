--- 
title: "Forecasting and Analytics with the Augmented Dynamic Adaptive Model (ADAM) - v2"
...
In comparison with the [published version of the book](https://openforecast.org/adam-v1/), this one has been expanded to include:

1. Some sections were expanded to provide more detailed explanations of approaches. This includes:
    a. Subsection \@ref(ETSANNvsMNN), which shows the difference between the additive and the multiplicative error ETS models;
    b. Subsection \@ref(ETSAAdN), discussing the damped trend trajectories in more detail;
    c. Section \@ref(ADAMETSMultiplicativeAlternative), which discusses the alternative multiplicative ETS formulation;
    d. Subsection \@ref(ADAMInitialisationARIMA), explaining a new initialisation of ADAM ARIMA;
    e. Subsection \@ref(ADAMMultipleFrequenciesExamplesARIMA), having more discussion on using non-Normal distributions with ARIMA;
    f. Subsection \@ref(oETSModelSelection), which introduces the DGPs produced by the oETS models;
    g. Subsection \@ref(forecastingADAMOtherExample), showing a more adequate visualisation for the cumulative over the lead time forecasts;
2. Fixes in errors and typos.
3. It relies on the most recent version of the `smooth` package for R, which means that the outputs in the book differ from the ones online.

I plan to expand this online monograph further by introducing a new ADAM ETS formulation and a separate chapter on distributions for ADAM. This information will be available here as soon as the book is amended.
