...
\begin{equation*}
  \begin{aligned}
    ...
    \mathrm{V}(y_{t+h}|t) = & \left( \sum_{i=1}^d \left(\mathbf{w}_{m_i}' \sum_{j=1}^{\lceil\frac{h}{m_i}\rceil-1} \mathbf{F}_{m_i}^{j-1} \mathbf{g}_{m_i} \mathbf{g}'_{m_i} (\mathbf{F}_{m_i}')^{j-1} \mathbf{w}_{m_i} \right) + 1 \right) \sigma^2
  \end{aligned},
\end{equation*}
where in our case $m_i$ corresponds to each lag in the vector $l$. Another thing to note that due to how we formulated the model, each $\mathbf{w}_{m_i}$ and $\mathbf{g}_{m_i}$ would contain only one non-zero element, while the matrix $\mathbf{F}_{m_i}$ would be sparse, containing only one non-zero column $i$.
...
\begin{equation}
  \begin{aligned}
    \mu_{\log y,t+h} = \mathrm{E}(\log y_{t+h}|t) = & \exp \left( \sum_{i=1}^d \left(\mathbf{w}_{m_i}' \mathbf{F}_{m_i}^{\lceil\frac{h}{m_i}\rceil-1} \right) \log \mathbf{v}_{t} \right) \\
    \mathrm{V}(\log y_{t+h}|t) = & \left( \sum_{i=1}^d \left(\mathbf{w}_{m_i}' \sum_{j=1}^{\lceil\frac{h}{m_i}\rceil-1} \mathbf{F}_{m_i}^{j-1} \mathbf{g}_{m_i} \mathbf{g}'_{m_i} (\mathbf{F}_{m_i}')^{j-1} \mathbf{w}_{m_i} \right) + 1 \right) \sigma^2
  \end{aligned}.
  (\#eq:ADAMARIMAMultiplicativeMomentsLogs)
\end{equation}
The obtained logarithmic moments can then be used to get the normal ones, after making assumptions about the distribution of the random variable. For example, if we assume that $\left(1+\epsilon_t \right) \sim \mathrm{log}\mathcal{N}\left(-\frac{\sigma^2}{2}, \sigma^2\right)$, then the conditional expectation and variance can be calculated as:
\begin{equation}
  \begin{aligned}
    & \mu_{y,t+h} = \mathrm{E}(y_{t+h}|t) = \exp( \left(\mathrm{E}(\log y_{t+h}|t) + \frac{\mathrm{V}(\log y_{t+h}|t)}{2} \right) \\
    & \mathrm{V}(\log y_{t+h}|t) = \left(\exp\left( \mathrm{V}(\log y_{t+h}|t) \right) - 1 \right)\exp\left(2\mathrm{E}(\log y_{t+h}|t) + \mathrm{V}(\log y_{t+h}|t) \right)
  \end{aligned}.
