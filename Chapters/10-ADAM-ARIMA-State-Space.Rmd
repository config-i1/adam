...
## ETS + ARIMA
Coming back to the topic of ETS and ARIMA, we can now look at it from the SSOE state space point of view. In general an ETS + ARIMA model can be formulated in the general form, which we have [already discussed](#ADAMETSPureAdditive) several times in this textbook:
\begin{equation*}
  \begin{aligned}
    &{y}_{t} = \mathbf{w}' \mathbf{v}_{t-\mathbf{l}} + \epsilon_t \\
    &\mathbf{v}_{t} = \mathbf{F} \mathbf{v}_{t-\mathbf{l}} + \mathbf{g} \epsilon_t
...
\end{equation}
In the transition part of the model \@ref(eq:ADAMETSARIMAANN011), the two equations duplicate each other, because they same exactly the same mechanism of update of states. In fact, as we know from [a previous chapter](#ARIMAETS011), ETS(A,N,N) and ARIMA(0,1,1) are equivalent, when $\alpha=1+\theta_1$. If we estimate this model, then we are duplicating the state, splitting it into two parts with some arbitrary weights, which can be written something like this:
\begin{equation}
  \begin{aligned}
    &{y}_{t} = w_1 l_{t-1} + w_2 v_{1,t-1} + \epsilon_t & \text{so that } w_1+w_2=1 \\
    &l_t = l_{t-1} + \alpha \epsilon_t \\
    &v_{1,t} = v_{1,t-1} + (1+\theta_1) \epsilon_t 
  \end{aligned}.
  (\#eq:ADAMETSARIMAANN011)
\end{equation}
And there is an infinite combination of weights $w_1$ and $w_2$ that would result to exactly the same fit. So, the model \@ref(eq:ADAMETSARIMAANN011) does not have a unique combination of parameters and thus is not identifiable.

This example shows that, when using ETS + ARIMA, modelling needs to be done with a caution, not to get an unreasonable model that cannot be identefied. As a general recommendation, keep the [ETS and ARIMA connection](#ARIMAandETS) in mind, when deciding, what to construct.
