...
## ETS + ARIMA
Coming back to the topic of ETS and ARIMA, we can now look at it from the SSOE state space point of view. A pure additive ETS + ARIMA model can be formulated in the general form, which we have [already discussed](#ADAMETSPureAdditive) several times in this textbook:
\begin{equation*}
  \begin{aligned}
    &{y}_{t} = \mathbf{w}' \mathbf{v}_{t-\mathbf{l}} + \epsilon_t \\
    &\mathbf{v}_{t} = \mathbf{F} \mathbf{v}_{t-\mathbf{l}} + \mathbf{g} \epsilon_t
...
This example shows that, when using ETS + ARIMA, model building needs to be done with care, not to get an unreasonable model that cannot be identefied. As a general recommendation, keep the [ETS and ARIMA connection](#ARIMAandETS) in mind, when deciding, what to construct. Here is a short list of model combinations that lead to identification problems and thus should be avoided:

1. ETS(A,N,N)+ARIMA(0,1,1);
2. ETS(A,A,N)+ARIMA(0,2,2);
3. ETS(A,Ad,N)+ARIMA(1,1,1);
4. ETS(A,N,A)+ARIMA(0,1,m)(0,1,0)$_m$;
5. ETS(A,A,A)+ARIMA(0,2,m+1)(0,1,0)$_m$;
6. ETS(A,Ad,A)+ARIMA(1,1,m+1)(0,1,0)$_m$.

When it comes to the multiplicative error and mixed ETS models, then the ETS+ARIMA might not have the same issues as the pure additive one. This is because the [multiplicative ETS](#ADAMETSPureMultiplicative) and [multiplicative ARIMA](#ADAMARIMAPureMultiplicative) are formulated differently. An example is an ETS(M,N,N)+ARIMA(0,1,1), which is formulated as:
\begin{equation}
  \begin{aligned}
...
4. ETS(M,N,M)+logARIMA(0,1,m)(0,1,0)$_m$;
5. ETS(M,M,M)+logARIMA(0,2,m+1)(0,1,0)$_m$;
6. ETS(M,Md,M)+logARIMA(1,1,m+1)(0,1,0)$_m$.
