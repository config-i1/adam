...
<!-- We extend the general ADAM framework discussed in the previous chapters with the introduction of the non-constant variance in the following way: -->
<!-- \begin{equation} -->
<!--   \begin{aligned} -->
<!--     & {y}_{t} = w(\mathbf{v}_{t-\mathbf{l}}) + r(\mathbf{v}_{t-\mathbf{l}}) \varepsilon_t \\ -->
<!--     & \varepsilon_t = \varsigma({\mathbf{v}_{\varsigma,t-\mathbf{l}}}) \epsilon_t \\ -->
<!--     & \mathbf{v}_{t} = f(\mathbf{v}_{t-\mathbf{l}}) + g(\mathbf{v}_{t-\mathbf{l}}) \epsilon_t -->
<!--   \end{aligned}, -->
<!--   (\#eq:ETSADAMStateSpaceScale) -->
<!-- \end{equation} -->
<!-- where the error term function $\varsigma({\mathbf{v}_{\varsigma,t-\mathbf{l}}})$ depends on its own states and set of explanatory variables. -->
