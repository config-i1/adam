...
y <- sim.ssarima(orders=list(i=1), lags=1, obs=120,
                 constant=10)
msarima(y$data, orders=list(i=1), constant=TRUE,
        h=10, holdout=TRUE) |>
    plot(which=7, main="")
```
...
Now, **why is it called "exponential"**? Because the same method can be represented in a different form, if we substitute $\hat{y}_{t}$ in right-hand side of \@ref(eq:BrownMethod):
\begin{equation*}
  \hat{y}_{t+1} = \hat{\alpha} {y}_{t} + (1 -\hat{\alpha}) \hat{y}_{t},
\end{equation*}
by the same formula for the previous step (i.e. shifting all time indices by one):
\begin{equation*}
