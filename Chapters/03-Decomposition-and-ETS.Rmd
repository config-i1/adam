...
Note that each textbook and paper might use slightly different names to refer to the abovementioned components. For example, in classical decomposition [@Persons1919], it is assumed that (1) and (2) jointly represent a "trend" component so that a model will contain error, trend and seasonality. This decomposition has modifications, which include cyclical component(s).
