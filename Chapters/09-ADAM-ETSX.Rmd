...
\begin{equation}
  \begin{aligned}
    ...
    \text{V}(y_{t+h}|t) = & \left((h-1) \alpha^2 + 1 \right) \sigma^2 + \sum_{i=1}^n a^2_i \sigma^2_{x_{i,h}} + \sum_{i\neq j, i=1}^n a_i a_j \sigma_{x_{i,h},x_{j,h}}
  \end{aligned},
...
\begin{equation}
  \begin{aligned}
    \mu_{y,t} = & \mathrm{E}\left(l_{t-1} + \sum_{i=1}^n a_i x_{i,t} + \epsilon_{t} \left. \right| t-1 \right) = \\
...
\begin{equation}
  \begin{aligned}
    \hat{y}_{t} = & \hat{l}_{t-1} + \sum_{i=1}^n \hat{a}_i \hat{x}_{i,t} \\
    e_t = y_t - \hat{y}_{t} \\
