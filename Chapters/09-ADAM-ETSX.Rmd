...
\end{equation}
which is equivalent to the combination of equations \@ref(eq:ETSXADAMStateSpacePureAdditiveMeasurement) and \@ref(eq:ETSXADAMStateSpacePureAdditiveTransition). The pure multiplicative model will have the same values for the state, lags, measurement and persistence vectors and transition matrix, but formulated via logarithms, similar to the [pure multiplicative ETS](#ADAMETSPureMultiplicative).
