...
### Conditional moments of the dynamic ETSX
...
\begin{equation}
  \mathbf{v}_{t+h-1} = & \mathbf{F}^{h-1} \mathbf{v}_{t} + \sum_{j=1}^{h-1} F^{h-1-j} \mathrm{diag}\left(\mathbf{w}_{t+j}\right)^{-1} \mathbf{g} \epsilon_{t+j} .
...
\begin{equation}
    {y}_{t+h} = \mathbf{w}'_{t+h} \mathbf{F}^{h-1} \mathbf{v}_{t} + \mathbf{w}'_{t+h} \sum_{j=1}^{h-1} F^{h-1-j} \mathrm{diag}\left(\mathbf{w}_{t+j}\right)^{-1} \mathbf{g} \epsilon_{t+j} + \epsilon_{t+h} .
...
\begin{equation}
  \begin{aligned}
    ...
    \text{V}(y_{t+h}|t) = \left(\mathbf{w}'_{t+h} \sum_{j=1}^{h-1} F^{h-1-j} \mathrm{diag}\left(\mathbf{w}_{t+j}\right)^{-1} \mathbf{g} \right)^2 \sigma^2 + \sigma^2
  \end{aligned} .
...
Wile the conditional expectation in the case of random explanatory variables is straightforward, the variance is more complicated, because it introduces complex interactions between variances of different variables and error term:
\begin{equation}
  \begin{aligned}
...
\end{equation}
where $\boldsymbol{\mu}'_{w,t+h}$ is the vector of conditional h steps ahead expectations for each element in the $\mathbf{w}_{t+h}$. In case of ETS components, the vector would contain ones.
