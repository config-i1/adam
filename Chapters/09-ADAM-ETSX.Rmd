...
## Conditional expectation and variance of ETSX {#ETSXConventionalConditionalMoments}
### The ETSX with known explanatory variables
...
Finally, in order to distinguish the ETSX with static parameters from the ETSX with dynamic ones, we will use the letters "S" and "D" in the names of models. So, the model \@ref(eq:ETSXADAMANNDeterministic) can be called ETSX(A,N,N){S}, while the model \@ref(eq:linearRegressionDynamicMultiplicative), assuming that $a_{0,t-1}=l_{t-1}$, would be called ETSX(M,N,N){D}. We use curly brackets in order to split the ETS states from the type of X. Furthermore, given that the model with static regressors is assumed in many contexts to be the default one, the ETSX(\*,\*,\*){S} model can also be denoted just ETSX(\*,\*,\*).


### Conditional moments of dynamic ETSX
...
In the case of random explanatory variables, the conditional expectation is straightforward and is similar to the one in the static ETSX model:
\begin{equation}
...
\end{equation}
where $\mathbf{D}_t=\mathbf{F} - \mathrm{diag}\left(\mathbf{w}_{t}\right)^{-1} \mathbf{g} \mathbf{w}_{t}'$ is the time varying discount matrix. The main issue in the case of dynamic ETSX is that the stability condition varies over time together with the values of explanatory variables. So, it is not possible to derive it for the general case. In order to make sure that the model is stable, we need for all eigenvalues of each $\mathbf{D}_{j}$ for all $j=\{1,\dots,t\}$ to lie in the unit circle.
...
As for the **forecastability** condition, for the ETSX{D} it should be (based on and the same logic as [in section 6.4](#stabilityConditionAdditiveError)):
\begin{equation}
...
## Dealing with categorical variables in ETSX
When dealing with categorical variables in regression context, they are typically expanded to a set of dummy variables. So, for example, having a variable colour that can be "red", "blue" and "green" for different observations $t$, we would expand it to three dummy variables, `colourred`, `colourblue` and `colourgreen`, each one of which is equal to 1, when the respective colour is observed and equal to zero otherwise. When including these variables in the model, we would drop one of them (which is sometimes called pivot variable) and have a model with two dummy variables of a type:
\begin{equation}
  y_t = a_0 + a_1 x_{1,t} + \dots + a_n x_{n,t} + d_1 colourred_t + d_2 colourblue_t + \epsilon_t,
  (\#eq:RegressionWithDummies)
\end{equation}
where $d_i$ is the parameter for the $i$-th dummy variable. The same procedure can be done in the context of ETSX, and the principles will be exactly the same for ETSX{S}. However, when it comes to the dynamic model, the parameters have time indeces, and there can be different ways of formulating the model. Here is the first one:
\begin{equation}
  \begin{aligned}
    y_{t} = & a_{0,t-1} + a_{1,t-1} x_{1,t} + \dots + a_{n,t-1} x_{n,t} + d_1 colourred_t + d_2 colourblue_t + \epsilon_t \\
    a_{i,t} = & a_{i,t-1} + \left \lbrace \begin{aligned}
                  &\delta_i \frac{\log(1+\epsilon_t)}{x_{i,t}} \text{ for each } i \in \{1, \dots, n\}, \text{ if } x_{i,t}\neq 0 \\
                  &0 \text{ otherwise }
            \end{aligned} \right. \\
    d_{1,t} = & d_{1,t-1} + \left \lbrace \begin{aligned}
                  &\delta_{n+1} \epsilon_t, \text{ if } colourred_t\neq 0 \\
                  &0 \text{ otherwise }
            \end{aligned} \right. \\
    d_{2,t} = & d_{2,t-1} + \left \lbrace \begin{aligned}
                  &\delta_{n+2} \epsilon_t, \text{ if } colourblue_t\neq 0 \\
                  &0 \text{ otherwise }
            \end{aligned} \right.
  \end{aligned} .
  (\#eq:ETSXDynamicDummies01)
\end{equation}
Here we assume that each specific category of the variable colour changes over time on its own with their own smoothing parameters $\delta_{n+1}$ and $\delta_{n+1}$. Alternatively, we can assume that they have the same smoothing parameters, implying that the changes of the parameters are similar throughout different categories of the variable:
\begin{equation}
  \begin{aligned}
    d_{1,t} = & d_{1,t-1} + \left \lbrace \begin{aligned}
                  &\delta_{n+1} \epsilon_t, \text{ if } colourred_t\neq 0 \\
...
\end{equation}
So, this comparison shows on one hand that the mechanism of ETSX{D} is natural for the ETS model, and on the other hand that using the same smoothing parameters for categorical variables can be a reasonable idea, especially in cases, when we do not have grounds to assume that each category of variable should evolve over time independently.


## Variables selection in ETSX
There are different approaches for [automatic variables selection](#modelSelection), but not all of them are efficient in the context of dynamic models. For example, backward stepwise might be either not feasible in case of small samples or may take too much time to converge to an optimal solution (it has polynomial computational time). This is because the ETSX model needs to be refitted and reestimated over and over again using recursive relations based, for example, on the state space model \@ref(eq:ADAMETSXPureAdditiveDynamicNonSeasonal). The classical stepwise forward might also be too slow, because it has polynomial computational time. So, there need to be some simplifications, which will make variables selection in ETSX doable in a reasonable time.

In order to make the mechanism doable in a limitted time, we rely on @Sagaert2021 approach of stepwise trace forward selection of variables. It is the approach that uses the partial correlations between variables in order to identify, which of the variables to include on each iteration, and has because of that linear computational time. Still, doing that in the proper ETSX would take more time than needed, so one of the possibles solutions is to do variables selection in ETSX in the following steps:
...
4. Estimate the ETSX model with the selected explanatory variables.
