...
\begin{equation}
  \mathbf{v}_{t} = \mathbf{D}_{1}^{t} \mathbf{v}_{0} + \sum_{j=0}^{t-1} \mathbf{D}_{t-j}^{j} y_{t-j},
...
\begin{equation}
  \lim\limits_{t\rightarrow\infty}\left(\mathbf{w}_{t}'\mathbf{D}_{1}^{t-1}\right) = \text{const} .
