...
\end{equation}
This case becomes useful in makeing the connection between the ETSX and the conventional seasonal ETS model. Let's assume that we deal with quarterly data with no trend and we have a categorical variable `quarterOfYear`, which can be `First`, `Second`, `Third` and `Fourth`, depending on the specific observation. Based on \@ref(eq:ETSXDynamicDummies02), the model can then be formulated as:
\begin{equation}
  \begin{aligned}
    y_{t} = & l_{t-1} + d_1 quarterOfYear_{1,t} + d_2 quarterOfYear_{2,t} + d_3 quarterOfYear_{3,t} + d_4 quarterOfYear_{4,t} + \epsilon_t \\
    l_t = l_{t-1} + \alpha \epsilon_t \\
    d_{i,t} = & d_{i,t-1} + \left \lbrace \begin{aligned}
