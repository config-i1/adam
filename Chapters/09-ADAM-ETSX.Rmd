...
In order to distinguish the ETSX with static parameters and the ETSX with dynamic ones, we will use the letters "S" and "D" in the name of models. So, the model \@ref(eq:ETSXADAMANNDeterministic) would be called ETSX(A,N,N;S), while the model \@ref(eq:linearRegressionDynamicMultiplicative), assuming that $a_{0,t-1}=l_{t-1}$, would be called ETS(M,N,N;D). We use semicolon in order to split the ETS states from the type of X. Given that the model with static regressors is assumed in many contexts to be the default one, the ETSX(\*,\*,\*;S) model can also be called just ETSX(\*,\*,\*).
