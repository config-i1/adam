...
\begin{equation}
  \begin{aligned}
    ...
    a_{i,t} = & a_{i,t-1} + \left \lbrace \begin{aligned}
                  &\delta_i \frac{\log(1+\epsilon_t)}{x_{i,t}} \text{ for each } i \in \{1, \dots, n\}, if x_{i,t}\neq 0 \\
                  &0 \text{ otherwise }
            \end{aligned} \right.
  \end{aligned} .
...
In order to distinguish the ETSX with static parameters and the ETSX with dynamic ones, we will use the letters "S" and "D" in the name of models. So, the model \@ref(eq:ETSXADAMANNDeterministic) can be called ETSX(A,N,N;S), while the model \@ref(eq:linearRegressionDynamicMultiplicative), assuming that $a_{0,t-1}=l_{t-1}$, would be called ETSX(M,N,N;D). We use semicolon in order to split the ETS states from the type of X. Given that the model with static regressors is assumed in many contexts to be the default one, the ETSX(\*,\*,\*;S) model can also be denoted just ETSX(\*,\*,\*).


### Conditional moments of the dynamic ETSX
Similar to how it was discussed in [the previous section](#ETSXConventionalConditionalMoments), we can have two cases in the dynamic model: (1) when explanatory variables are assumed to be known, (2) when explanatory variables are assumed to be random. For illustrative purposes, we will use ETSX(A,N,N;D) model, keeping in mind that other pure additive models can be easily used instead. The cases of other models are not discussed in this part in detail - the moments for these models need to be calculated based on simulations.
