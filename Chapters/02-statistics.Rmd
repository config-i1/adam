...
We will use different estimation techniques throughout this book, one of the main of which is **Maximum Likelihood Estimate** (MLE). The very rough idea of the approach is to maximise the chance that each observation in the sample follow a pre-selected distribution with specific set of parameters.

### Mathematical explanation
We will explain this idea on an example of [normal distribution](#distributionsNormal) and a simple linear regression model:
\begin{equation}
    y_t = \mu_{y,t} + \epsilon_t = a_0 + a_1 x_{1,t} + \epsilon_t ,
    (\#eq:MLESimpleRegression)
\end{equation}
where $a_0$ and $a_1$ are the population parameters (the true model parameters). The typical assumption in regression context is that $\epsilon_t \sim \mathcal{N}(0, \sigma^2)$, which means that $y_t \sim \mathcal{N}(\mu_{y,t}, \sigma^2)$. We can use this assumption in order to calculate the point likelihood value for each observation based on the [PDF of Normal distribution](#distributionsNormal):
\begin{equation}
    \mathcal{L} (a_0, a_1, \sigma^2 | y_t) = f(y_t | a_0, a_1, \sigma^2) = \frac{1}{\sqrt{2 \pi \sigma^2}} \exp \left( -\frac{\left(y_t - \mu_{y,t} \right)^2}{2 \sigma^2} \right).
    (\#eq:MLEPointLik)
\end{equation}
Very roughly, what the value \@ref(eq:MLEPointLik) shows is the chance that the data comes from the assumed model with specified parameters (we know that the data does not come from a model, but this interprertatio is easier to work with). This is not the same as probability, because for the continuous random variables the probability for it to be equal to any specific number is equal to zero, but has a similar idea. The point likelihood \@ref(eq:MLEPointLik) is not very helpful on its own, but we can get $T$ values like that, based on our sample of data. We can then summarise it in one number, that would characterise the whole sample, given the assumed distribution, model and selected values of parameters:
\begin{equation}
    \mathcal{L} (\boldsymbol{\theta} | \mathbf{y}) = \mathcal{L} (a_0, a_1, \sigma^2 | \mathbf{y}) = \prod_{t=1}^T f(y_t | a_0, a_1, \sigma^2),
    (\#eq:MLEFullLik)
\end{equation}
where $\boldsymbol{\theta}$ is the vector of all parameters in the model. We take the product in \@ref(eq:MLEFullLik) because we need to see the joint likelihood for all observations and because we can typically assume that the point likelihoods are independent of each other (for example, the value on observation $t$ will not be influenced by the value on $t-1$). The value \@ref(eq:MLEFullLik) shows the summary chance that the data comes from the assumed model with specified parameters. Having this value, we can change the values oif parameters of the model, getting different value of \@ref(eq:MLEFullLik). And using an iterative procedure, we can get such estimates of parameters that would maximise the likelihood \@ref(eq:MLEFullLik), which are called Maximum Likelihood Estimates (MLE) of parameters. However, working with the products in that formula is difficult, so typically we linearise it using natural logarithm, obtaining log-likelihood:
\begin{equation}
    ...
    (\#eq:MLEFullLogLik)
\end{equation}
Based on that, we can find some of parameters of the model analytically. For example, we can take derivative of \@ref(eq:MLEFullLogLik) with respect to the scale $\hat{\sigma}^2$ (which is just an estimate of the true parameter $\sigma^2$) and equate it to zero in order to find the value that maximises the log-likelihood function:
\begin{equation}
...
\end{equation}
which is in fact a Mean Squared Error of the model. So, if we calculate the value of $\hat{\sigma}^2$ using the formula \@ref(eq:MLEFullLogLikScale), we will maximise the likelihood with respect to the scale parameter. In fact, we can insert \@ref(eq:MLEFullLogLikScale) in \@ref(eq:MLEFullLogLik) in order to obtain the so called concentrated (or profile) log-likelihood for the normal distribution:
\begin{equation}
    \ell^* (\boldsymbol{\theta}, \hat{\sigma}^2 | \mathbf{y}) = -\frac{T}{2}\left( \log(2 \pi e) + \log \hat{\sigma}^2 \right) .
    (\#eq:MLEFullLogLikConcentrated)
\end{equation}
This function is useful because it simplifies some calculations and also demonstrates the condition, for which the likelihood is maximised: the first part on the right hand side of the formula does not depend on the parameters of the model, it is only the $\log \hat{\sigma}^2$ that does. So, the maximum of the concentrated log-likelihood \@ref(eq:MLEFullLogLikConcentrated) is obtained, when $\hat{\sigma}^2$ is minimised, implying the minimisation of MSE, which is also called "Ordinary Least Squares" (OLS)
) estimation. So, we have just demonstrated that if we assume normality in the model, then the estimates of parameters $\hat{a}_0$ and $\hat{a}_1$ obtained via the maximisation of the likelihood conicide with the values obtained from OLS. So, why bother with MLE, when we have OLS?

First, the finding above holds for normal distribution only. If we assume a different [distribution](#distributions), we would get a different estimates of parameters. In some cases, it might not be possible or reasonable to use OLS, but MLE would be a plausible option.

Second, the MLE of parameters have good statistical properties: they are [consistent](#estimatesPropertiesConsistency) and [efficient](#estimatesPropertiesEfficiency). These properties hold almost universally under very mild conditions. Note that they are not necessarily [unbiased](#estimatesPropertiesBias), but after estimating the model, one can de-bias some estimates of parameters (for example, calculate the standard deviation of the error via devision of the sum of squared erros by the number of degrees of freedom $T-k$ instead of $T$).

Third, likelihood can be used for the model assessment, even when the standard statistics, such as $R^2$ or $F-test$ are not available.

Finally, it permits the model selection via information criteria. This is not possible unless you assume a distribution and maximise the respective likelihood. In some cases, we can see that AIC is calculated for the models estimated via OLS, but there is still an assumption of normality behind it.

Note that the MLE assumes that all parameters are estimated, so it typically has more parameters to estimate than, for example, OLS. This is discussed in some detail later in the [next section](#statisticsNumberOfParameters).

### An example in R
In order to further explain the MLE idea, we consider a simple example, when we want to estimate the model $y_t = \mu_y + \epsilon_t$, assuming normality for the $\epsilon_t$. In this case we want to estimate two parameters using ML: $\hat{\mu}_y$ and $\hat{\sigma}^2$. First, we generate the random variable in R:
```{r}
y <- rnorm(100, 100, 10)
```

If on some iteration we get values of parameters $\hat{\mu}_y=80$ and $\hat{\sigma}=10$, then this implies that we fit the following normal curve to the data:
...
```
and we get the following log-likelihood value:
```{r}
...
Now, on Figure \@ref(fig:MLENormalExample02), the normal curve is much closer to the data, but still a bit off. The log-likelihood value in this case is `r sum(dnorm(y,90,10,log=T))`, which is higher than the previous one, indicating that we are moving towards the maximum of the likelihood function. Moving further, setting $\hat{\mu}_y=100$, we get:
...
Figure \@ref(fig:MLENormalExample02) demonstrates a much better fit than in the previous cases with the log-likelihood of `r sum(dnorm(y,100,10,log=T))`, which is even higher than in the previous case. We are almost there. In fact, in order to maximise this likelihood, we just need to calculate the mean of the variable (this is the MLE of the location parameters in normal distribution) and insert it in the function to obtain:
...
So the value of $\hat{\mu}_y=$`r round(mean(y),2)` maximises the likelihood function, resultin in log-likelihood of `r sum(dnorm(y,mean(y),10,log=T))`.

In a similar fashion we can get the MLE of the scale parameter $\sigma^2$ of the model. In this case, we will be changing the height of the distribution. Here is an example with $\hat{\mu}_y=\bar{y}=$`r round(mean(y),2)` (where $\bar{y}$ is the sample mean) and $\hat{\sigma}=15$:
...
```
Figure \@ref(fig:MLENormalExample05) demonstrates that the curve is located lower than needed, which implies that the scale parameter $\hat{\sigma}$ is too high. The log-likelihood value in this case is `r sum(dnorm(y,mean(y),15,log=T))`. In order to get a better fit of the curve to the data, we need to reduce the $\hat{\sigma}$. Here how the situation would look for the case of $\hat{\sigma}=10$:
```{r MLENormalExample06, fig.cap="ML example with Normal curve and $\hat{\mu}_y=\bar{y}$ and $\hat{\sigma}=10$"}
hist(y, xlim=c(50,150), main="", probability=TRUE)
lines(c(50:150),dnorm(c(50:150),mean(y),10),col="blue2",lwd=2)
abline(v=mean(y),col="blue2",lwd=2)
```

The fit on Figure \@ref(fig:MLENormalExample06) is better than on Figure \@ref(fig:MLENormalExample05), which is also reflected in log-likelihood value being equal to `r sum(dnorm(y,mean(y),10,log=T))` instead of `r sum(dnorm(y,mean(y),15,log=T))`. The best fit and the maximum of the likelihood is obtained, when the scale parameter is estimated using the formula $\hat{\sigma}^2 = \frac{1}{T}\sum_{t=1}^T\left(y_t - \bar{y}\right)^2$, resulting in log-likelihood of `r sum(dnorm(y,mean(y),sqrt(mean((y-mean(y))^2)),log=T))`. Note that if we use the unbiased estimate of the variance $\hat{s}^2 = \frac{1}{T-1}\sum_{t=1}^T\left(y_t - \bar{y}\right)^2$, the log-likelihood will not reach the maximum and will be equal to `r sum(dnorm(y,mean(y),sd(y),log=T))`. You can check this in R via the following commands:
```{r, eval=FALSE}
# The maximum log-likelihood with the biased variance
sum(dnorm(y,mean(y),sqrt(mean((y-mean(y))^2)),log=TRUE))
# The log-likelihood value with the unbiased variance
sum(dnorm(y,mean(y),sd(y),log=TRUE))
```

Finally, if the data comes from a completely different distribution, and we fit the normal one, the idea stays the same: to find the parameters of the normal distribution, that would guarantee the best possible fit. Here is an example with MLE of parameters of Normal distribution for the data following Log Normal one:
...
which has the log likelihood of `r sum(dlnorm(y,mean(log(y)),sd(log(y)),log=TRUE))`. This gives us an idea that it is possible to compare different distributions via the likelihood, finding the better fit to the data, which is furhter explored in the [next section](#modelSelection).

As a final word, when it comes to more complicated models with more parameters and dynamic structure, the specific curves and data become more complicated, but the logic of the fit stays the same.
