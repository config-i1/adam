...
nIterations <- 500
obs <- 100
meanValues <- vector("numeric",obs)
medianValues <- vector("numeric",obs)
y <- rnorm(100000,mu,sigma)
for(i in 1:obs){
    ySample <- replicate(nIterations,sample(y,i*100))
    meanValues[i] <- var(apply(ySample,2,mean))
    medianValues[i] <- var(apply(ySample,2,median))
}
```
...
```{r eval=FALSE}
options(scipen=6)
plot(1:100*100,meanValues/medianValues, type="l", xlab="Sample size",ylab="Relative efficiency", ylim=range(c(1,variancesRatios)))
abline(h=1, col="red")
```
...
\begin{equation}
    \ell(\mathbf{Y}| \theta, \sigma^2) = - \frac{T}{2} \log \left(2 \pi \sigma^2\right) - \sum_{t=1}^T \frac{\left(y_t - \mu_t \right)^2}{2 \sigma^2} ,
    (\#eq:NormalLogLik)
\end{equation}
...
\begin{equation}
    ...
    y_t = \mu_t \times (1+\epsilon_t) \sim \mathcal{IG}\left(\mu_t, \frac{s}{\mu_t} \right),
...
\begin{equation}
    \hat{s} = \frac{1}{T} \sum_{t=1}^T \frac{\left(\epsilon_t \right)^2}{1+\epsilon_t} .
