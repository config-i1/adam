...
\begin{equation}
    \text{V}(y) = \frac{1}{T} \sum_{j=1}^T \left( y_t - \bar{y} \right)^2,
    (\#eq:varianceBiased)
\end{equation}
...
\begin{equation}
    ...
    (\#eq:sigmaNormal)
\end{equation}
where $T$ is the sample size. \@ref(eq:sigmaNormal) coincides with Mean Squared Error (MSE), discussed in the [section 1](#errorMeasures).
...
\begin{equation}
    \hat{s} = \frac{1}{T} \sum_{t=1}^T \left| y_t - \mu_{y,t} \right| .
    (\#eq:sLaplace)
\end{equation}
...
\begin{equation}
    \hat{s} = \frac{1}{2T} \sum_{t=1}^T \sqrt{\left| y_t - \mu_{y,t} \right|} ,
    (\#eq:sS)
\end{equation}
...
\begin{equation}
    \hat{s} = \sqrt[^{\beta}]{\frac{\beta}{T} \sum_{t=1}^T\left| y_t - \mu_{y,t} \right|^{\beta}},
    (\#eq:sGND)
\end{equation}
...
\begin{equation}
    \hat{s} = \frac{1}{T} \sum_{t=1}^T \left(y_t - \mu_{y,t} \right)(\alpha - I(y_t \leq \mu_{y,t})) .
    (\#eq:sALaplace)
\end{equation}
...
\begin{equation}
    \hat{s} = \frac{1}{T} \sum_{t=1}^T \frac{\epsilon_t^2}{1+\epsilon_t} .
