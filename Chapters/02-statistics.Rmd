...
lines(seq(-5,5,0.1), dnorm(seq(-5,5,0.1)), lwd=1, lty=1, col="darkblue")
lines(c(-5,5), c(0,0), col="black", lwd=1)
polygon(c(seq(-5,qnorm(0.025),0.1),rev(seq(-5,qnorm(0.025),0.1))),
        c(dnorm(seq(-5,qnorm(0.025),0.1)), rep(0,length(seq(-5,qnorm(0.025),0.1)))), col="grey")
polygon(c(seq(qnorm(0.975),5,0.1),rev(seq(qnorm(0.975),5,0.1))),
        c(dnorm(seq(qnorm(0.975),5,0.1)), rep(0,length(seq(qnorm(0.975),5,0.1)))), col="grey")
...
## Hypothesis testing
While we will not need hypothesis testing for ADAM itself, we might face it in some parts of the textbook, so it is worth discussing the main aspects of it.

Hypothesis testing arises naturally from the idea of sampling: we cannot have all the data in the world, we work with just a sample of it, and we want to get an idea about some parameters of the true model (in the population), based on the data that we have. The classical example is the hypothesis testing about the population mean, based on the sample values of mean and variance.
