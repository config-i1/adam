...
\end{equation}
where $\sigma^2$ is the variance of the error term. However, the covariance between $y_t$ and $y_{t-2}$ will be equal to zero for the pure MA(1) (given that the [usual assumptions](#assumptions) hold). In fact, @Box1976 showed that for the MA(q) ACF tells more about the order of the model than for the AR(p): it will drop rapidly right after the specific lag q.


### Partial autocorrelation function (PACF)
The other instrument useful for the time series analysis with respect to ARIMA is called "partial ACF". The idea of this follows from ACF directly. As we have spotted, if the process we deal with follows AR(1), then $\rho(2)=\phi_1^2$ just because of the temporal element. In order to get rid of this effect, when calculating the correlation between $y_t$ and $y_{t-2}$ we could take off the correlation $\rho(1)$. This type of correlation is called "partial" and we will denote it as $\varrho(\tau)$. While there are proper formulae for calculating this value, we will not provide and discuss them here, it is important just to understand the idea of PACF. If we get the correlation, clean of the temporal element, then PACF would drop abruptly right after the lag p for the AR(p) process.



Note, however that the idea discussed in this section is not necessarily bi-directional: e.g. if we deal with MA(q), ACF drops abruptly right after the lag q, but if ACF drops abruptly after the lag q, then this does not necessarily mean that we are dealing with MA(q). The former follows directly from the assumed "true" model, while the latter refers to the identification of the model onthe data, and there can be different reasons for the ACF to behave in a way it does.
