...
\begin{equation}
  y_t \Delta^0(B^4) \varphi^1(B^4) \Delta^1(B) \varphi^1(B) = \epsilon_t \vartheta^1(B^4) \vartheta^1(4) .
...
```{r}
x <- vector("numeric", 20)
x[1:5] <- c(97,87,85,94,95)
phi <- c(0.3,0.5)
for(i in 6:20){
    x[i] <- phi[1] * x[i-1] + phi[2] * x[i-4] + phi[1] * phi[2] * x[i-5]
}
plot(x, type="l", xlab="horizon", ylab="Forecast")
```
...
\begin{equation}
  y_t \Delta^D_n(B^m_n) \varphi^P_n(B^m_n) \dots \Delta^D_1(B^m_1) \varphi^P_1(B^m_1) \Delta^d(B) \varphi^p(B) = \epsilon_t \vartheta^Q_n(B^m_n) \dots \vartheta^Q_1(B^m_1) \vartheta^q(B) ,
  (\#eq:MSARIMACompact)
\end{equation}
where $n$ is the number of seasonal cycles. The slightly more compact and less comprehensible form of \@ref(eq:MSARIMACompact) is:
\begin{equation}
  y_t \prod_{j=0}^n \Delta^D_j (B^m_j) \varphi^P_j(B^m_j) = \epsilon_t \prod_{j=0}^n \vartheta^Q_j(B^m_j) ,
  (\#eq:MSARIMACompactFinal)
\end{equation}
taking that $D_0=d$, $P_0=p$, $Q_0=q$. Conceptually, the model \@ref(eq:MSARIMACompactFinal) is neat, as it captures all the complex relations in the data, but it is not easy to understand and work with, not to mention the potential estimation and order selection problems.
