...
Note that typically, the scale, shape and shift parameters that maximise the likelihood are biased and do not coincide with the ones typically used in the OLS. For exampl in case of normal distribuiton, OLS estimate has $T-k$ in the denominator, while the likelihood one has just $T$.
...
```{example}
For example, if we know that the parameter $b_1$ lies between 0 and 1, and in the estimation process it gets to the value of 1 (due to how the optimiser works), it can be considered as a restriction $b_1=1$. So, when estimated via the minimum of MSE with the restriction, this means that $k=1$.
```
...
```{example}
For example, if in the same simple linear regression $b_1 = \frac{b_0}{\sigma^2}$, then the number of all the estimated parameter via the maximum of likelihood is 2: $b_0$ and $\sigma^2$.
```
