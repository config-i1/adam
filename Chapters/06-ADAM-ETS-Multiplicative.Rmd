...
Finally, when it comes to conditional quantiles, the same term $\log(\mathbf{1}_k + \mathbf{g}_{m_i} \epsilon_{t+j})$ causes different set of problems, introducing convolutions of products of random variables. To better understand this issue, consider persistence part of the equation for the ETS(M,N,N) model, which is:
\begin{equation}
