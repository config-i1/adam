...
The oETS$_F$ model produces the straight line for the probability of `r round(oETSFModel$initial$level,2)`, ignoring that in our example, the probability of occurrence has increased over time.
...
if(knitr:::is_latex_output()){
    ...
    knitr::asis_output('(\\#eq:probabilityFunction)')
...
Practically speaking, using oETS$_D$ makes sense, when you expect the demand on your product either to disappear completely or to become regular. In other cases, oETS$_I$ and oETS$_O$ could be used efficiently. On M5 data, I found the latter two models to performed very well in the majority of cases [@Svetunkov2019a].


### Model selection in oETS {#oETSModelSelection}
Before we move to the model selection in oETS, it might make sense to see what sort of probability occurrence the models above are suitable for. These are summarised visually in Figure \@ref(fig:oETSModelSeries), where the random errors were generated in exactly the same fashion (with a fixed random seed) and $\alpha=0.4$ to show the differences in dynamics between the processes.

```{r oETSModelSeries, fig.cap="Example of several probability of occurrence processes according to oETS.", echo=FALSE}
pFix <- ts(rep(0.5, 10000))

set.seed(41)
pOdds <- sim.oes("MNN", obs=10000, persistence=0.4, occurrence="odds",initial=1,
                 randomizer="rlnorm", meanlog=-0.5*0.1^2, sdlog=0.1)
set.seed(41)
pInv <- sim.oes("MNN", obs=10000, persistence=0.4, occurrence="inverse",initial=1,
                randomizer="rlnorm", meanlog=-0.5*0.1^2, sdlog=0.1)
set.seed(41)
pDir <- sim.oes("MNN", obs=10000, persistence=0.4, occurrence="direct",initial=0.5,
                randomizer="rlnorm", meanlog=-0.5*0.1^2, sdlog=0.1)
set.seed(41)
pGenA <- sim.oes("MNN", obs=10000, persistence=0.2, occurrence="general", initial=1,
                randomizer="rlnorm", meanlog=-0.5*0.1^2, sdlog=0.1,
                initialB=1, persistenceB=0.4)
set.seed(41)
pGenB <- sim.oes("MNN", obs=10000, persistence=0.4, occurrence="general", initial=1,
                randomizer="rlnorm", meanlog=-0.5*0.1^2, sdlog=0.1,
                initialB=1, persistenceB=0.2)

par(mfcol=c(3,2), mar=c(2,2,3,1))
plot(pFix, ylim=c(0,1), ylab="Probability", main="Fixed probability")
plot(pOdds, ylim=c(0,1), ylab="Probability", main="Odds ratio")
plot(pInv, ylim=c(0,1), ylab="Probability", main="Inverse Odds ratio")
plot(pDir, ylim=c(0,1), ylab="Probability", main="Direct probability")
plot(pGenA, ylim=c(0,1), ylab="Probability", main="General model, example 1")
plot(pGenB, ylim=c(0,1), ylab="Probability", main="General model, example 2")
```

Figure \@ref(fig:oETSModelSeries) demonstrates visually what was discussed in the previous subsections related to the demand occurrence:

1. The fixed probability model implies no change over time;
2. The odds ratio one implies the inevitable demand obsolescence (in mathematics this corresponds to the probability almost surely converging to zero);
3. The inverse odds ratio demonstrates the demand building up asymptotically (almost surely converging to one);
4. The direct probability shows asymptotic behaviour similar to the oETS$_O$, but with more rapid changes;
5. And finally, the general one shows a variety of behaviours depending on the specific values of parameters for models a and b.

Hopefully, this gives a better understanding of the differences between the oETS models.

Now, when it comes to selecting the most appropriate one, there are two dimensions for the model selection in oETS:
...
or in logarithms:
\begin{equation}
    \ell(\boldsymbol{\theta} | \textbf{y}) = \sum_{o_t=1} \log(p_t) + \sum_{o_t=0} \log(1-p_t) + \sum_{t=1}^T \log f_z(z_t | \mathbf{v}_{z,t-\boldsymbol{l}}),
...
entropiesTable <- c("$\\epsilon_t \\sim \\mathcal{N}(0, \\sigma^2)$",
                    ...
                    "$\\frac{1}{2}\\left(\\log \\left(\\pi e \\sigma^2\\right) -\\log(2) \\right)$",
                    # Gamma distribution
...
if(knitr:::is_latex_output()){
    ...
    knitr::asis_output('(\\#eq:VarianceOfProduct)')
    knitr::asis_output('\\end{equation}')
```
...
if(knitr:::is_latex_output()){
    ...
    knitr::asis_output('(\\#eq:iETSConditionalVariance01)')
    knitr::asis_output('\\end{equation}')
```
