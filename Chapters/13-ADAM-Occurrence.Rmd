...
Finally, in some situations the occurrence might be not random, i.e. we deal with not an intermittent demand, but with a demand, where we can predict perfectly when sales will happen. An example of such situation is demand on watermelons, which becomes equal to zero in specific periods of time in Winter. If we know when the demand occurs, we can use the respective values of zeroes and ones in the variable $o_t$, simplifying the model and derivations from it. This is implemented in both `adam()` and `alm()` functions, where the user needs to provide the vector of zeroes and ones in the `occurrence` parameter.
