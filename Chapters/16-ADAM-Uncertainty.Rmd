...
Before we move to the discussion of parameters uncertainty and how it propagates to the states, fitted values and final forecasts, it makes sense to understand how data can be generated based on some estimate ADAM.
...
```
This matrix does not provide helpful analytical information but demonstrates that the estimates of the initial level and trend of the `r adamETSBJ$model` model applied to this data are negatively correlated. The values from the covariance matrix can then be used for various purposes, including calculation of confidence intervals of parameters, construction of confidence interval for the fitted value and point forecasts and finally, the construction of more adequate prediction intervals.
...
```
Based on these values and the formula \@ref(eq:confidenceInterval), we can produce confidence intervals for parameters of any ADAM, which is done in R using the `confint()` method. For example, here are the intervals for the significance level of 1%:
```{r}
...
adamETSAir <- adam(AirPassengers, "MMM", h=10, holdout=TRUE)
adamETSAirRefitted <- reapply(adamETSAir, nsim=1000)
plot(adamETSAirRefitted)
```
...
The `adamETSAirRefitted` object contains several variables, including:
...
```{r eval=FALSE}
y <- rnorm(200,100,10)
adamARIMAy1 <- adam(y, "NNN", h=10, holdout=TRUE,
                    orders=c(0,1,1))
adamARIMAyRefitted1 <- reapply(adamARIMAy1)
plot(adamARIMAyRefitted1)
```
...
```{r eval=FALSE}
adamARIMAy2 <- adam(y, "NNN", h=10, holdout=TRUE,
                    orders=c(0,1,1))
adamARIMAyRefitted2 <- reapply(adamARIMAy2, bootstrap=TRUE,
                               nsim=1000, parallel=TRUE)
plot(adamARIMAyRefitted2)
```
