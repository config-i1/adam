...
In the `smooth` package for R, there is a `multicov()` method that allows extracting the multiple steps ahead covariance matrix $\Sigma$. The method can estimate the covariance matrix using analytical formulae (where available), or via empirical calculations (based on multiple steps ahead in-sample error), or via simulation. Here is an example for one of models in R:
