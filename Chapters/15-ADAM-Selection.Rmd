...
9. What about the demand occurrence part of the model? (this question has already been answered in Section \@ref(oETSModelSelection)).

In this chapter, we discuss all aspects, related to model selection and combinations in ADAM. We will start the discussion with principles based on [information criteria](#informationCriteria), we will then move to more complicated topics, related to pooling and then we will finish with selection and combinations based on rolling origin.
...
```{r}
adamETSARIMAModel <- adam(AirPassengers, model="PPP",
                          orders=list(ar=c(3,3),i=c(2,1),ma=c(3,3),select=TRUE),
                          silent=TRUE, h=10, holdout=TRUE, initial="back")
adamETSARIMAModel
...
2. Median, which produces good combinations, when the pool of models is limited and might contain those that produce forecasts very different from the others (e.g. explosive forecasts);
3. Trimmed mean
4. Winsorised mean
5. Weighted mean
...
@Claeskens2016 provides explanation, why weighted combinations do not always increase the accuracy.

@Jose2008 consider trimmed, winsorized means together with simple averages and median
