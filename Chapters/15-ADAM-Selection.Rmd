...
6. Pool of the safe models, which corresponds to the one from the `ets()` function from the `forecast` package: `model="SSS"` with an option of restricting it further with the none/additive trend, i.e. `model="SXS"` and other options for components, similar to (3) and (4);
7. Manual pool of models, which can be provided as a vector of models, for example: `model=c("ANN","MNN","ANA","AAN")`.
