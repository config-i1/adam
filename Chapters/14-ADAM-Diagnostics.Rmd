...
In this chapter, we investigate how ADAMs can be diagnosed and improved. Most topics will build upon the typical model assumptions discussed in Subsection \@ref(assumptions) and in Section 12 of @SvetunkovSBA. Some of the assumptions cannot be diagnosed properly, but there are well-established instruments for others. We will consider the following assumptions and discuss how to check whether they are violated or not:
...
3. [The explanatory variables are not correlated with anything but the response variable](#diagnosticsMulticollinearity) (technically speaking, this is not an assumption about the model):
...
```{r adamSeat01, fig.cap="Basic regression model for the data of Road Casualties in Great Britain 1969–84."}
...
plot(adamSeat05, 2, level=0.95, main="")
```
...
```{r}
adamSeat07 <- adam(SeatbeltsWithOutliers, "MNM", lags=12,
                   formula=drivers~PetrolPrice+kms+
                     law+outlier8)
AICc(adamSeat07)
```
...
<!-- This model is slightly worse than the one with all outliers in terms of AICc, but this could be for purely technical reasons. If we cannot motivate or predict them, we should not include them as dummy variables. Model 7 with higher AICc looks more reasonable than the previous models from this point of view. A similar diagnostics plot can be done for the new model (Figure \@ref(fig:adamSeat07Outliers)): -->

```{r adamSeat07Outliers, fig.cap="Standardised residuals vs Fitted for the model 7.", include=FALSE, eval=FALSE}
plot(adamSeat07, 2, main="")
```

<!-- We could continue the exploration by introducing other dummy variables based on the plot in Figure \@ref(fig:adamSeat07Outliers). However, in general, we should not do that unless we have a good reason for adding more variables (e.g. we know that the model did not capture something that happened). -->
...
par(mfcol=c(1,2), mar=c(4,4,0,1))
plot(adamSeat12, 10:11, level=0.99, main="")
```
...
This new constructed model is SARIMAX(1,0,0)(0,1,1)$_{12}$, which is the model we achieved manually. Note that it is even better than the `r adamSeat05$model` (model 5) from the previous section in terms of AICc. Its residuals are much better behaved than the ones of model 5 (although we might need to analyse the residuals for the potential outliers, as discussed in Subsection \@ref(diagnosticsOutliers)):
...
So for the purposes of analytics and forecasting, we would be inclined to use SARIMAX(2,0,0)(0,1,1)$_{12}$ rather than `r adamSeat05$model`.
...
spread(cbind(
  as.data.frame(abs(resid(adamSeat03))),
...
```{r adamSeat05Spread, fig.cap="Spread plot of absolute residuals vs variables included in the model 5."}
spread(cbind(
...
lines(seq(-400,400,1),
      dnorm(seq(-400,400,1), 0, sd(residuals(adamSeat03))),
      col="red")
```
...
There are also formal tests for the distribution of residuals, such as Shapiro-Wilk [@WikipediaShapiroWilk2021] and Kolmogorov-Smirnov [@WikipediaKolmogorovSmirnov2021]. The former tests the hypothesis that residuals follow Normal distribution. The latter is much more flexible and allows comparing the empirical distribution with any other one (theoretical or empirical). However, I prefer to use visual inspection when possible instead of these tests because, as discussed in Section 5.3 of @SvetunkovSBA, the null hypothesis is always wrong. It will inevitably be rejected with the increase of the sample size, which does not mean that it is either correct or wrong. Besides, if you fail to reject H$_0$, it does not mean that your variable follows the assumed distribution. It only means that you have not found enough evidence to reject the null hypothesis.
