...
\begin{equation}
    ...
    (\#eq:logGARCHARMA)
\end{equation}
which can be obtained by substituting $\log \sigma_{t-j}^2 = \log \epsilon_{t-j}^2 - \log \eta_{t-j}^2$ for all $j$. Given the connection of logARMA with logGARCH and the discussion in Subsection \@ref(ADAMARIMAPureMultiplicative), the model \@ref(eq:logGARCHARMA) is just a special case of the scale model for ADAM, being a special case of \@ref(eq:ADAMSMGeneral).
...
Finally, Francq (2013) (not sure, check) argue that given the relation between GARCH and ARMA, the latter can be applied to the data, if its parameters are restricted so that $\sum_{j=1}^p a_j + \sum_{j=1}^q b_j < 0$ and $a_j, b_j \in (0,1)$ for all $j$. This means that ADAM ARIMA model can also be applied to the data as long as these restrictions are satisfied.
