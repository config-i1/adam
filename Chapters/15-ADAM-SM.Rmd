...
Coming back to the original GARCH model, @Bollerlev1986 argue that GARCH, being equivalent to ARMA, will be a stationary process if its parameters are restricted so that $\sum_{j=1}^p a_j + \sum_{j=1}^q b_j < 0$ and $a_j, b_j \in [0,1)$ for all $j$. For ADAM ARIMA, this means that the model can also be applied to the data without logarithmic transform as long as these restrictions are satisfied.
