...
\begin{equation}
	...
	(\#eq:LogNormalLikelihood_doesnt_occur)
\end{equation}

Based on the equations \@ref(eq:LogNormalLikelihood_occurs) and \@ref(eq:LogNormalLikelihood_doesnt_occur), we can summarise the likelihood for the whole sample of $T$ observations:
\begin{equation}
...
\end{equation}
The main issue in calculating the likelihood \@ref(eq:LogLikelihoodForDGP) is that the demand sizes are not observable, when $o_t=0$. This means that we cannot calculate the likelihood using the conventional approach, we need to use something else. @Svetunkov2019a proposed using Expectation Maximisation (EM) algorithm for this purposes, which is typically done in the following stages:
...
\end{equation}
Luckily, the expectation in \@ref(eq:LogLikelihoodForDGPExpectation) is well known in statistics and called "Differential Entropy" (it is actually negative differential entropy in the formula above). It will differ from one distribution to another, depending on what demand sizes model is assumed. Table \@ref(tab:differentialEntropy) summarises differential entropies for the distributions used in ADAM.
...
# GN distribution
                    ...
                    "$\\frac{1}{\\beta}-\\log\\left(\\frac{\\beta}{2*s*\\Gamma\\left(\\frac{1}{\\beta}\\right)}\\right)$",
...
# Gamma distribution
                    ...
                    "$s^{-1} + \\log\\left(\\Gamma\\left(s^{-1}\\right)\\right) + \\left(1-s^{-1}\\right)*\\psi\\left(s^{-1}\\right)$",
# logN distribution
...
The final thing to note is that all the iETS models estimated via the maximisation of likelihood will produce [consistent](#estimatesPropertiesConsistency), [efficient](#estimatesPropertiesEfficiency), but [positively biased](#estimatesPropertiesBias) estimates of the smoothing parameters (higher than needed). @Svetunkov2019a gives an explanation to this phenomenon. Given the connection between MLE for some distributions and MSE (see Section \@ref(MSEandMAEEstimators) for detail), this also implies that such forecasting methods as Croston [@Croston1972] and TSB [@Teunter2011] would also result in positively biased estimates of parameters, when estimated via the minimisation of MSE.
