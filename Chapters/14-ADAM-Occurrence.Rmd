...
The occurrence part of the model oETS$_O$ is constructed using the very same `oes()` function, but also allows specifying the ETS model to use. For example, here is the oETS$_O$(M,Md,N) model:
```{r oETSOExample1}
oETSOModel <- oes(y, model="MMdN", occurrence="odds-ratio", h=10, holdout=TRUE)
oETSOModel
plot(oETSOModel)
```
...
The function `oes()` implements the oETS$_I$ model as well. For example, here is the oETS$_I$(M,Md,N) model:
```{r oETSIExample1}
oETSIModel <- oes(y, model="MMdN", occurrence="inverse-odds-ratio", h=10, holdout=TRUE)
oETSIModel
plot(oETSIModel)
```
...
Here's an example of the application of the oETS$_D$(M,Md,N) to the same artificial data:
```{r oETSDExample1}
oETSDModel <- oes(y, model="MMdN", occurrence="d", h=10, holdout=TRUE)
oETSDModel
plot(oETSDModel)
```
...
<!-- Finally, Kourentzes & Athanasopoulos paper on intermittent demand -->
