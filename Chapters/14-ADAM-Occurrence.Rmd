...
### General iETS model, iETS$_G$
Uniting the models iETS$_O$ with iETS$_I$, we can obtain the "general" model, which in the most general case can be summarised in the following way:
\begin{equation}
    \begin{aligned}
...
\begin{equation} \label{eq:ISSETSMNNEstimated}
	\begin{aligned}
...
The experiments that I have done so far show that oETS$_G$ very seldomly brings improvements in comparison with iETS$_O$ or iETS$_I$ in terms of forecasting accuracy. Besides, selecting models for each of the parts is a challenging task. So, this model is theoretically nice, being more general than the other iETS models, but is not very practical. Still it is useful because we can introduce different iETS model by restricting $a_t$ and $b_t$. For example, we can get:

1. iETS$_F$, when $\mu_{a,t} = \text{const}$, $\mu_{b,t} = \text{const}$ for all $t$;
2. iETS$_O$, when $\mu_{b,t} = 1$ for all $t$;
3. iETS$_I$, when $\mu_{a,t} = 1$ for all $t$;
4. iETS$_D$, when $\mu_{a,t} + \mu_{b,t} = 1$, $\mu_{a,t} \leq 1$ for all $t$ (discussed in the next subsection);
5. iETS$_G$, when there are no restrictions.
