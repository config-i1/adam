...
### An example in R
In order to see how the error measures work, we consider the following example based on a couple of forecasting functions from the `smooth` package for R [@Hyndman2008b and @Svetunkov2015] and measures from `greybox`:
```{r eval=FALSE}
# Generate the data
y <- rnorm(100,100,10)
...
The plot in Figure \@ref(fig:ROProcessAnimation) shows how the origin moves forward and the point and interval forecasts of the model change. As a result, this procedure gives information about the performance of the model over a set of observations, not on one random observation.
