...
1. The smoothing parameters are left intact, because they typicall lie between 0 and 1, where 0 means that the respective states are not updated over time. Damping parameter is also left as is, shrinking it to zero and enforcing no trend.
...
## Initialisation of ADAM
In order to construct a model, we need to initialise it, meaning define the values of $\mathbf{v}_{-m+1}, \dots, \mathbf{v}_0$ - initial states of the model. There are different ways of doing that, but here we will only discuss the following three:

1. Optimisation of initials,
2. Backcasting,
3. Provided values.

(1) implies that the values of initials are found in the same procedure as other parameters of the model. (2) means that the initials are refined iteratively, when the model is fit to the data from observation $t=1$ to $t=T$ and backwards. Finally, (3) is when a user know initials and provides them to the model.

As a side note, we assume in ADAM that the model is initialised at the moment just before $t=1$, we do not believe that it was initialised some time before the Big Bang (as ARIMA typically does) and we do not initialised it at the start of the sample. This way we make all models comparable, making them work on exactly the same sample, no matter how many differences are taken or how many seasonal components they contain.


### Optimisation vs backcasting
...
The more complex the selected model is, the more parameters it will have to estimate, and all of this will happen in one and the same iterative process in the optimiser:
...
6. Go to (2) and repeat until a specific criteria is met.

The stopping criteria can be different and specified by the user. There are several things considered in the optimiser of `adam()`:

1. Maximum number of iterations, which is equal to $40\times k$, where $k$ is the number of all estimated parameters,
2. The relative precision of the optimiser with default value of $10^{-6}$, which regulates the relative change of parmaters;
3. The absolute precision of the optimiser with default value of $10^{-8}$, which regulates the absolute change of parmaters;
4. The stopping criterion in case of the relative change in the loss function with default value of $10^{-8}$;

All these parameters are explained in more detail in the documentation of `nloptr()` function from `nloptr` package for R. `adam()` accepts several other stopping criteria, see the documentation for details.

The mechanism explained above can become quite complicated if a big complex model is constructed and might take a lot of time and manual tuning of parameters in order to get to the optimum. And in some cases it is worth considering to reduce the number of estimated parameters. One of the ways of doing so is the backcasting.
...
The backcasting procedure implies the extended fitting process for the model, removing the need to estimate all the initials. It works especially well, when you work with large samples of data (thousands of observations) and with models with several seasonal components. The bigger your model is, the more time optimisation will take and the more likely backcasting would become more appropriate.

Nonetheless, no matter what initialisation method is used, we need to start the fitting process from $t=1$, and this cannot be done unless we provide some pre-initialised values for parameters. The better we guess the correct values, the faster the model will converge to the optimum. `adam()` uses several heuristics in this stage, explained in more detail in the next subsections.
