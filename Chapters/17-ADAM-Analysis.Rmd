...
This model seems to be better than the previous ones in terms of AICc, so we would be inclined to use it for further inference. We can explore other outliers and see if they happened at random or not. For example, the outlier on observation 156 looks suspicious. However, we do not know what happened there and cannot investigate, so we will stop on this model.
