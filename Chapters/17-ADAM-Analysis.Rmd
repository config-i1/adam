...
```
Based on this, we would be inclined to select the multiplicative model. My personal judgment in this specifica case agrees with the information criterion. 
...
We see that there is no specific pattern in the outliers, they happen randomly, so they appear not because of the omitted variables or wrong transformations. We have 8 observations lying outside the bounds, which given that the sample size of 192 observations, means that the 95% interval contains $\frac{192-8}{192} \times 100 \mathrm{\%} \approx 95.8\mathrm{\%}$ of observations. This is close to the nominal 95%, but it might make sense to further investigate some of outliers.
...
text(time(Seatbelts[,"drivers"])[adamModelSeat05Outliers$id],
     Seatbelts[adamModelSeat05Outliers$id,"drivers"]+100,
     adamModelSeat05Outliers$id, col="red")
```
...
setNames(c(AICc(adamModelSeat05), AICc(adamModelSeat06)),
         c("ETSX","ETSXOutliers"))
```
Comparing the two values above, I would conclude that the dummy variables do not help and that we should not include all outliers in the data. Istead of including all of them, we can try the model with the outlier for the observation 170, which corresponds to the eighth outlier (as this one was the main suspect):

```{r}
adamModelSeat07 <- adam(SeatbeltsWithOutliers,"MNM",lags=12,
                        formula=drivers~PetrolPrice+kms+front+rear+law+outlier8)
plot(adamModelSeat07,2)
adamModelSeat07
```

However this model is not better than the previous one (in terms of AICc) and has a different set of outliers. So, we can probably come to conclusion that the outliers in our case happen at random and can be ignored.
...
In our case, the function neglects all the outliers, agreeing with what we have found previously. Given that this is an automated approach, it is prone to potential mistakes and needs to be treated with care as it might select unnecessary dummy variables and lead to overfitting. I would still recommend exploring the outliers manually, when possible and not to rely too much on the automated procedures.
