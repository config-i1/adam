...
```{r}
adamLocationModel <- adam(SeatbeltsData, "MNM", h=12, holdout=TRUE,
...
spread(cbind(resid=as.vector(resid(adamLocationModel)),
             as.data.frame(SeatbeltsData)[1:180,-1]))
```

The only slightly alarming element in the plot on Figure \@ref(fig:adamModelETSXMNMDiagnosticsXreg) is the slight change in variance for the variable `law`. Based on this, we will investigate the potential effect of law on the scale of the model:

```{r}
adamScaleModel <- sm(adamLocationModel, model="NNN", formula=drivers~law)
```
...
```{r}
adamModelMerged <- implant(adamLocationModel, adamScaleModel)
```
...
```{r}
setNames(c(AICc(adamLocationModel),AICc(adamModelMerged)),
         c("Location","Implanted"))
```
As we see the model with the implanted scale model has a higher AICc, which means that the introduction of the explanatory variable `law` in the scale model does not bring value and that the variable should not be used.


### Example 2
The example in the previous subsection shows how the analysis can be done for the scale model. In order to see how the scale model can be used for forecasting, we will consider another example, with dynamic elements. For this example, we will use `AirPassengers` and an automatically selected ETS model with Inverse Gaussian distribution (for demonstration purpose):

```{r}
adamLocationModel <- adam(AirPassengers, h=10, holdout=TRUE,
...
```{r}
adamScaleModel <- sm(adamLocationModel, model="YYY")
```

The `sm` function accepts roughly the same set of parameters as `adam()`, when applied to the objects of `adam` class. So, you can experiment with other types of ETS/ARIMA models if you want. Here is what the function has selected automatically for the scale model:
...
plot(forecast(adamModelMerged$scale,
              h=10, interval="prediction"))
```

The point forecast of scale for the next `h` observations can also be used to produce more appropriate prediction intervals for the merged model. This will be discussed in the Chapter \@ref(ADAMForecasting).
