...
\end{equation}
The left hand side of \@ref(eq:speedDistanceModelSqrt4) contains the ratio of change of distance to change of speed, when the change of speed is close to zero. This is in a way is a definition of time from the classical $speed = \frac{distance}{time}$ formula from physics. The problem in our case is that the distance is measured in feet, while the speed is measured in mph. It would be convenient to convert them both to meters per second. This can be done in the following way:
\begin{equation}
  \begin{aligned}
    & distance = \frac{distanceMetric}{2.2369} \\
    & speed = \frac{speedMetric}{3.2808}
  \end{aligned}
    (\#eq:speedDistanceMetric)
\end{equation}
Inserting \@ref(eq:speedDistanceMetric) in \@ref(eq:speedDistanceModelSqrt4), we get:
\begin{equation}
    \frac{\mathrm{d}distanceMetric}{\mathrm{d}speedMetric} = \frac{2.2369}{3.2808} \times 2 (a_1 speed + \epsilon) \approx 1.3636 (a_1 speed + \epsilon).
    (\#eq:speedDistanceModelSqrt4)
\end{equation}
Based on this we can now say that with the increase of speed of car by 1mph its stopping time on average will increase by `r round(coef(slmSpeedDistanceModel05),4)*1.3636` seconds. Yes, this is a very crude interpretation, but this is the closest we can get to something meaningful from this model. This effect would vary in 95% of cases from `r round(confint(slmSpeedDistanceModel05)[2],2)*1.3636` to `r round(confint(slmSpeedDistanceModel05)[3],2)*1.3636` seconds, depending on various circumstances (type of car, slipperiness of the road, breaks condition etc).
