...
Subjectively, I would say that Model 5 is the most appropriate from all the models under consideration: it corresponds to the physical model on one hand, and has constant variance on the other one. Its parameter contains some average information about the mass of cars and their breaking forces (this is based on the formula \@ref(eq:speedDistanceFormula)). The interpretation of the parameter in this model, however, is challenging.
<!-- In order to get to some crude interpretation, we need to revert to maths. Model 5 can be written as: -->
<!-- \begin{equation} -->
<!--     distance = (a_1 speed + \epsilon)^2 . -->
<!--     (\#eq:speedDistanceModelSqrt3) -->
<!-- \end{equation} -->
<!-- In order to get to the understanding of the meaning of parameter, we will take first derivative of distance with respect to speed: -->
<!-- \begin{equation} -->
<!--     \frac{\mathrm{d}distance}{\mathrm{d}speed} = 2 (a_1 speed + \epsilon) . -->
<!--     (\#eq:speedDistanceModelSqrt4) -->
<!-- \end{equation} -->
<!-- The left hand side of \@ref(eq:speedDistanceModelSqrt4) contains the ratio of change of distance to change of speed (acceleration), when the change of speed is close to zero. This is in a way is a definition of time from the classical $speed = \frac{distance}{time}$ formula from physics. Given that we deal with derivatives, this will be the change of time. The problem in our case is that the distance is measured in feet, while the speed is measured in mph. It would be convenient to convert them both to meters per second. This can be done in the following way: -->
<!-- \begin{equation} -->
<!--   \begin{aligned} -->
<!--     & distance = \frac{distanceMetric}{3.2808} \\ -->
<!--     & speed = \frac{speedMetric}{2.2369} -->
<!--   \end{aligned} -->
<!--     (\#eq:speedDistanceMetric) -->
<!-- \end{equation} -->
<!-- Inserting \@ref(eq:speedDistanceMetric) in \@ref(eq:speedDistanceModelSqrt4), we get: -->
<!-- \begin{equation} -->
<!--     \frac{\mathrm{d}distanceMetric}{\mathrm{d}speedMetric} = \frac{3.2808}{2.2369} \times 2 (a_1 speed + \epsilon) \approx 2.9333 (a_1 speed + \epsilon). -->
<!--     (\#eq:speedDistanceModelSqrt4) -->
<!-- \end{equation} -->
<!-- Based on this we can now say that with the increase of speed of car by one meter per second (which is 2.2369mph) the change of its stopping time on average will increase by `r round(coef(slmSpeedDistanceModel05),4)*2.9333` seconds. Yes, this is a very crude interpretation, but this is the closest we can get to something meaningful from this model. This effect would vary in 95% of cases from `r round(confint(slmSpeedDistanceModel05)[2],2)*2.9333` to `r round(confint(slmSpeedDistanceModel05)[3],2)*2.9333` seconds, depending on various circumstances (type of car, slipperiness of the road, breaks condition etc). -->


<!-- ### Types of variables transformations -->
<!-- Summarising this section, we discuss several possible types of transformations of variables in regression models. -->
