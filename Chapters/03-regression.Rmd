...
### Hypothesis testing
Another way to look at the uncertainty of parameters is to test a statistical hypothesis. As it was discussed in Section \@ref(hypothesisTesting), I personally think that hypothesis testing is a less useful instrument for these purposes, which might be misleading in some circumstances. Nonetheless, it has its merits and can be helpful if an analyst knows what they are doing. In order to test the hypothesis, we need to follow the procedure, described in Section \@ref(hypothesisTesting).

The classical hypothesis for the parameters is formulated in the following way:
\begin{equation}
    \begin{aligned}
        \mathrm{H}_0: & a_j = 0 \\
        \mathrm{H}_1: & a_j \neq 0
