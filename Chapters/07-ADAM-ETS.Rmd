...
where $\mu_t = \mathbf{w}' \mathbf{v}_{t-\mathbf{l}}$, $\sigma^2$ is the variance of the error term in logarithms and the $-\frac{\sigma^2}{2}$ appears due to the restriction $\text{E}(\epsilon_t)=0$. The restriction cannot be simply implemented for log versions of Laplace, S and Generalised Normal distributions (because their expectations do not have closed forms), so we do not discuss them here.
