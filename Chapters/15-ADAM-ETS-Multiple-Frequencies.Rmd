...
y <- zoo(forecast::taylor,
         order.by=as.POSIXct("2000/06/05")+(c(1:length(y))-1)*60*30)
plot(y)
```
...
In the code above we use the initialisation via [backacasting](#ADAMInitialisationOptAndBack), because otherwise the calculation will take much more time. The resulting model produces the forecasts that are off. This is mainly because the smoothing parameter $\alpha$ is too high and the model changes the level to frequently. We can see that in the plot of the state:
...
```{r}
adamModelETSXMNNGTMSE <- adam(taylorData, "MNN", h=336, holdout=TRUE,
...
```{r}
adamModelETSXMNMGTMSE <- adam(taylorData, "MNM", lags=48, h=336, holdout=TRUE,
                              initial="back", loss="GTMSE", formula=y~x1)
