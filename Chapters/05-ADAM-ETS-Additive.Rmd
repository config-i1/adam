...
\end{equation}
I use spaces in equations \@ref(eq:ETSADAMAA) to show how the matrix form is related to the general one. The positions of $l_{t-1}$, $b_{t-1}$ and $s_{t-m}$ correspond to the ones in the matrices in \@ref(eq:ETSADAMAAAMatrixForm). Now we can give names to each matrix and vector, for example:
\begin{equation}
  \begin{aligned}
