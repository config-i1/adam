...
\begin{equation}
  \begin{split}
    y_{t} &= \hat{y}_t (1 + e_t) \\
    \hat{y}_{t+1} &= \hat{y}_t (1 + \hat{\alpha} \frac{y_t - \hat{y}_t}{\hat{y}_t})
  \end{split} .
...
This example demonstratesonce again the difference between the forecasting method and the forecasting model. When we use SES, we ignore the distributional assumptions, which restricts the usage of the method.
...
\begin{equation}
    \hat{y}_{t+h|t} = (l_{t} + h b_t) s_{t+h-m\lceil\frac{h}{m}\rceil},
  (\#eq:ETSAANForecast)
\end{equation}
